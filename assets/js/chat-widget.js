...
(function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    const userName =
      typeof leviAgent.userName === "string" ? leviAgent.userName : "";
    const userInitial =
      typeof leviAgent.userInitial === "string" && leviAgent.userInitial
        ? leviAgent.userInitial
        : getInitial(userName, "U");
    const userAvatarUrl =
      typeof leviAgent.userAvatarUrl === "string"
        ? leviAgent.userAvatarUrl
        : "";
    const leviAvatarUrl =
      typeof leviAgent.leviAvatarUrl === "string"
        ? leviAgent.leviAvatarUrl
        : "";
    ...
    if (expand) {
      expand.addEventListener("click", function () {
        const isEnabled = window_.classList.contains(
          "levi-chat-window-fullwidth",
        );
    ...
    function sendMessage() {
      ...
      if (isEdit && editingMessageEl) {
        ...
        if (
          nextSibling &&
          nextSibling.classList.contains("levi-message-assistant")
        ) {
          nextSibling.remove();
        }
    ...
    async function sendMessageSSE(text, activityStream, phaseTimers, isEdit) {
      ...
      try {
        ...
        if (error.name === "AbortError") {
          ...
          addMessage(
            "❌ Entschuldigung, es gab einen Fehler: " + error.message,
            "assistant",
          );
    ...
    function handleSSEEvent(data, activityStream, phaseTimers) {
      ...
      switch (data.type) {
        ...
        case "done":
          ...
          var isFallback =
            serverMsg.indexOf("konnte keinen sauberen") !== -1 ||
          ...
          if (!isDuplicate) {
            ...
            var lastMsg = messages.querySelector(
              ".levi-message-assistant:last-of-type .levi-message-main",
            );
            if (lastMsg && !lastMsg.querySelector(".levi-usage-badge")) {
              lastMsg.insertAdjacentHTML(
                "beforeend",
                formatUsageBadge(data.usage),
              );
            }
          }
    ...
    function sendMessageClassic(text, activityStream, phaseTimers, isEdit) {
      ...
      fetch(leviAgent.restUrl + "chat", {
        ...
        .then((data) => {
          ...
          if (data.error) {
            addMessage(
              "❌ " + formatApiError(data.error, data.__httpStatus),
              "assistant",
            );
            return;
          }
          ...
          const cleanedMessage = sanitizeAssistantMessage(
    ...
    window.addEventListener("beforeunload", function (e) {
      if (sendInFlight || uploadInFlight) {
        e.preventDefault();
        var msg =
          "Levi arbeitet gerade an einer Aufgabe. Ein Seitenwechsel oder Neuladen würde den Prozess unterbrechen. Wirklich verlassen?";
        e.returnValue = msg;
        return msg;
      }
    });
    ...
    function uploadSelectedFiles(fileListObj) {
      ...
      fetch(leviAgent.restUrl + "chat/upload", {
        ...
        .then((data) => {
          ...
          const fullList = Array.isArray(data.session_files)
            ? data.session_files
            : null;
          if (fullList) {
          ...
          if (uploaded.length > 0) {
            uploadedFiles =
              fullList || uploadedFiles.concat(uploaded).slice(-5);
            renderUploadedFiles();
          }
      ...
      uploadedFiles.forEach((f) => {
        ...
        if (isImage && preview) {
          thumb =
            '<img src="' +
            escapeHtml(preview) +
            '" class="levi-chat-file-chip-thumb" alt="">';
        } else {
          const icon = isImage
        ...
        chip.innerHTML =
          thumb +
          '<span class="levi-chat-file-chip-name">' +
          name +
          "</span>" +
          '<button type="button" class="levi-chat-file-chip-remove" data-file-id="' +
          fileId +
          '" title="Entfernen">×</button>';
        fileList.appendChild(chip);
      });
      fileList
        .querySelectorAll(".levi-chat-file-chip-remove")
        .forEach((btn) => {
    ...
    function clearSessionFiles() {
      ...
      fetch(
        ...
        .catch((error) => {
          addMessage(
            "❌ Uploads konnten nicht gelöscht werden: " + error.message,
            "assistant",
          );
        });
    }
    ...
    function removeSessionFile(fileId) {
      ...
      fetch(
        ...
        .catch((error) => {
          addMessage(
            "❌ Datei konnte nicht entfernt werden: " + error.message,
            "assistant",
          );
        });
    }
    ...
    function clearCurrentSession() {
      ...
      fetch(leviAgent.restUrl + "chat/" + encodeURIComponent(sessionId), {
        ...
        .catch((error) => {
          addMessage(
            "❌ Session konnte nicht gelöscht werden: " + error.message,
            "assistant",
          );
        });
    }
    ...
    function formatUsageBadge(usage) {
      ...
      return (
        '<span class="levi-usage-badge" title="Prompt: ' +
        (usage.prompt_tokens || 0).toLocaleString("de-DE") +
        " | Completion: " +
        (usage.completion_tokens || 0).toLocaleString("de-DE") +
        " | Cached: " +
        cached.toLocaleString("de-DE") +
        " | API-Calls: " +
        calls +
        '">' +
        parts.join(" · ") +
    ...
    function addMessage(text, role, attachments, timestamp, usage, webSearch) {
      ...
      if (role === "user") {
        ...
        if (hasBadges) {
          ...
          if (webSearch) {
            inner +=
              '<span class="levi-msg-file levi-msg-web-search">' +
          ...
          if (attachments && attachments.length > 0) {
            attachments.forEach(function (f) {
              ...
              inner +=
                '<span class="levi-msg-file">' + thumb + name + "</span>";
            });
          }
          inner += "</div>";
        }
      ...
      if (role === "user") {
        html +=
          '<button type="button" class="levi-message-edit-btn dashicons dashicons-edit" title="Nachricht bearbeiten" aria-label="Nachricht bearbeiten"></button>';
      }
      html += "</div>";
    ...
    class ActivityBubble {
        ...
        if (phase === "start" || phase === "preview") {
          ...
          if (targetCard) {
            targetCard.className =
              "levi-tool-card " +
              (success ? "levi-tool-card-done" : "levi-tool-card-failed");
            var iconEl = targetCard.querySelector(".levi-tool-icon");
            if (iconEl) {
              iconEl.className =
                "levi-tool-icon " +
                (success ? "levi-tool-done" : "levi-tool-failed");
      ...
      showPlan(data) {
        ...
        toolList.forEach(function (t) {
          toolsHtml +=
            '<li><span class="dashicons dashicons-arrow-right-alt2"></span> ' +
            escapeHtml(t) +
            "</li>";
        });
    ...
    function formatApiError(message, httpStatus) {
      ...
      if (httpStatus === 401 || httpStatus === 403) {
        return (
          "Berechtigung/Nonce ungültig. Seite neu laden und erneut versuchen. (" +
          msg +
          ")"
        );
      }
      ...
      if (httpStatus === 503) {
        return (
          "Provider/Modell aktuell nicht verfügbar. Bitte anderes Modell wählen oder später erneut versuchen. (" +
          msg +
          ")"
        );
      }
    ...
    function buildAvatarHtml(role) {
      ...
      if (imgUrl) {
        imageHtml =
          ...
          '" alt="' +
          escapeHtml(title) +
          '" class="levi-message-avatar-image" loading="lazy" decoding="async" onerror="this.style.display=\'none\'; this.parentElement.classList.add(\'levi-avatar-fallback-visible\');">';
      }

      return (
        '<div class="levi-message-avatar levi-message-avatar-' +
        (isAssistant ? "assistant" : "user") +
        '">' +
        imageHtml +
        '<span class="levi-message-avatar-fallback">' +
        escapeHtml(letter) +
        "</span>" +
        "</div>"
      );
    ...
    if (testBtn) {
      testBtn.addEventListener("click", function () {
        ...
        fetch(leviAgent.ajaxUrl, {
          ...
          body:
            "action=levi_test_connection&nonce=" +
            encodeURIComponent(leviAgent.adminNonce),
        })
          .then((r) => r.json())
          .then((data) => {
            if (data.success) {
    ...
    if (repairBtn) {
      repairBtn.addEventListener("click", function () {
        ...
        fetch(leviAgent.ajaxUrl, {
          ...
          body:
            "action=levi_repair_database&nonce=" +
            encodeURIComponent(leviAgent.adminNonce),
        })
          .then((r) => r.json())
          .then((data) => {
    ...
    if (reloadBtn) {
      reloadBtn.addEventListener("click", function () {
        ...
        fetch(leviAgent.ajaxUrl, {
          ...
          body:
            "action=levi_reload_memories&nonce=" +
            encodeURIComponent(leviAgent.adminNonce),
        })
          .then((r) => r.json())
          .then((data) => {
            ...
            if (data.success) {
              ...
              const hadChanges =
                (r.changed_identity || []).length > 0 ||
                (r.changed_reference || []).length > 0;
              result.innerHTML =
                ' <span style="color: ' +
                (errors.length ? "orange" : "green") +
                ';">✅ ' +
                data.data.message +
                "</span>";
              if (hadChanges && errors.length === 0 && loadedCount > 0) {
                const warningEl = document.getElementById(
                  "levi-memory-changes-warning",
                );
                if (warningEl) {
    ...
    if (runSnapshotBtn) {
      runSnapshotBtn.addEventListener("click", function () {
        ...
        const progressWrap = document.getElementById(
          "levi-state-snapshot-progress-wrap",
        );
        const progressBar = document.getElementById(
          "levi-state-snapshot-progress",
        );
        ...
        fetch(leviAgent.ajaxUrl, {
          ...
          body:
            "action=levi_run_state_snapshot&nonce=" +
            encodeURIComponent(leviAgent.adminNonce),
        })
          .then((r) => r.json())
          .then((data) => {
