...
class ExecuteTool extends AbstractTool
{
    ...
    public function getParameters(): array
    {
        return [
            ...
            "command" => [
                "type" => "string",
                "description" =>
                    'WP-CLI Befehl (für type=cli, z.B. "plugin list --status=active")',
            ],
            "timeout" => [
                "type" => "integer",
                "default" => 30,
    ...
    private function executeWp(array $params): array
    {
        ...
        // Verbotene Funktionen (wie bei executePhp)
        $forbidden = [
            "exec",
            "system",
            "passthru",
        ...
        // Safe wrapper: write to temp file instead of eval()
        $wrapped = "<?php\n" . $code;
        $tempFile = tempnam(sys_get_temp_dir(), "levi_exec_wp_");
        if ($tempFile === false) {
            return [
                "success" => false,
                "error" => "Konnte temporäre Datei nicht erstellen",
