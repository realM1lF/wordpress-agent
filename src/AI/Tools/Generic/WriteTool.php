...
class WriteTool extends AbstractTool
{
    ...
    public function getParameters(): array
    {
        return [
            ...
            "meta" => [
                "type" => "object",
                "description" =>
                    "Meta-Daten als Key-Value-Paare (für Post, User)",
            ],
        ];
    }
    ...
    private function writePost(string $type, array $params): array
    {
        if (
            ...
            return [
                "success" => false,
                "error" =>
                    "Keine Berechtigung zum Erstellen/Bearbeiten von Posts",
            ];
        }

        $postData = [
            "post_type" =>
                $type === "page" ? "page" : $params["post_type"] ?? "post",
        ...
        if (!empty($params["title"])) {
            $postData["post_title"] = sanitize_text_field(
                (string) $params["title"],
            );
        }
    ...
    private function writeOption(array $params): array
    {
        ...
        if (
            in_array($name, $dangerous, true) &&
            !current_user_can("manage_options")
        ) {
            return [
                "success" => false,
                "error" => "Option '{$name}' erfordert manage_options.",
            ];
        }
    ...
    private function writeUser(array $params): array
    {
        ...
        if (!empty($params["username"])) {
            $userData["user_login"] = sanitize_user(
