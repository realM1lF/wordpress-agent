...
class ManageElementorTool extends AbstractTool {
    ...
    public function getName(): string { return 'manage_elementor'; }
    ...
    private function handleTemplate(string $action, array $params): array {
        ...
        switch ($action) {
            case 'list':
                ...
                return ['success' => true, 'items' => array_map(fn($t) => [
                    'id' => $t->ID,
                    'title' => $t->post_title,
                    'type' => get_post_meta($t->ID, \Elementor\TemplateLibrary\Source_Local::TYPE_META_KEY, true),
                    'date' => $t->post_date,
                ], $templates)];
            ...
            case 'get':
                ...
                return ['success' => true, 'data' => [
                    ...
                    'type' => get_post_meta($template->ID, \Elementor\TemplateLibrary\Source_Local::TYPE_META_KEY, true),
                    'meta' => $meta,
                ]];
            ...
            case 'create':
                $postData = [
                    'post_type' => $source,
                    'post_title' => sanitize_text_field((string) ($params['title'] ?? 'New Template')),
                    'post_status' => 'publish',
                ...
                if (!empty($params['type'])) {
                    update_post_meta($id, \Elementor\TemplateLibrary\Source_Local::TYPE_META_KEY, (string) $params['type']);
                }
            ...
            case 'update':
                ...
                if (!empty($params['title'])) {
                    $updateData['post_title'] = sanitize_text_field((string) $params['title']);
                }
            ...
            default:
                return ['success' => false, 'error' => "Unbekannte Template-Aktion: {$action}"];
        }
    }

    private function handlePage(string $action, array $params): array {
        if ($action !== 'edit_with_elementor') {
            return ['success' => false, 'error' => "Seiten-Aktion '{$action}' nicht unterstützt. Nutze 'edit_with_elementor'."];
        }
        ...
        if (!empty($params['elements'])) {
            ...
            if (is_array($elements)) {
                update_post_meta($pageId, '_elementor_data', wp_slash(json_encode($elements)));
            }
        }
    ...
    private function handleWidget(string $action, array $params): array {
        if ($action !== 'get_registered') {
            return ['success' => false, 'error' => "Widget-Aktion '{$action}' nicht unterstützt. Nutze 'get_registered'."];
        }
        ...
        return [
            'success' => true,
            'widgets' => array_slice($list, 0, min(100, (int) ($params['limit'] ?? 50))),
            'total' => count($list),
        ];
    }
