...
class ManageWooTool extends AbstractTool
{
    ...
    public function getParameters(): array
    {
        return [
            ...
            "type" => [
                "type" => "string",
                "description" =>
                    "Typ (für Produkt: simple/variable/grouped; Coupon: fixed_cart/percent)",
            ],
            "price" => [
                "type" => "number",
            ...
            "status" => [
                "type" => "string",
                "description" =>
                    "Status (für Bestellung: pending/processing/on-hold/completed/cancelled/refunded)",
            ],
            "code" => [
                "type" => "string",
            ...
            "key" => [
                "type" => "string",
                "description" =>
                    "WooCommerce-Einstellungs-Key (z.B. woocommerce_currency)",
            ],
            "value" => [
                "type" => "string",
    ...
    public function execute(array $params): array
    {
        if (!class_exists("WooCommerce")) {
            return [
                "success" => false,
                "error" => "WooCommerce ist nicht aktiv",
            ];
        }
    ...
    private function handleProduct(string $action, array $params): array
    {
        if (!function_exists("wc_get_product")) {
            return [
                "success" => false,
                "error" => "WooCommerce-Funktionen nicht verfügbar",
            ];
        }
        ...
        switch ($action) {
            ...
            default:
                return [
                    "success" => false,
                    "error" => "Unbekannte Produkt-Aktion: {$action}",
                ];
        }
    }
        ...
        switch ($action) {
            ...
            case "update_status":
                if (empty($params["id"]) || empty($params["status"])) {
                    return [
                        "success" => false,
                        "error" => "Erforderlich: id und status",
                    ];
                }
                $order = wc_get_order((int) $params["id"]);
                if (!$order) {
            ...
            default:
                return [
                    "success" => false,
                    "error" => "Unbekannte Bestellungs-Aktion: {$action}",
                ];
        }
    }
    ...
    private function handleCoupon(string $action, array $params): array
    {
        switch ($action) {
            ...
            default:
                return [
                    "success" => false,
                    "error" => "Unbekannte Coupon-Aktion: {$action}",
                ];
        }
    }
