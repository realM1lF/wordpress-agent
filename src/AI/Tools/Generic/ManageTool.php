...
class ManageTool extends AbstractTool {
    ...
    public function getName(): string { return 'manage'; }
    ...
    public function getParameters(): array {
        return [
            ...
            'item' => [
                'type' => 'object',
                'description' => 'Menü-Item {title, url, type(custom/post_type), object_id}',
            ],
            'item_id' => [
                'type' => 'integer',
    ...
    public function checkPermission(): bool {
        return current_user_can('manage_categories') || current_user_can('upload_files') || current_user_can('edit_posts');
    }
    ...
    private function handleTaxonomy(string $action, array $params): array {
        ...
        if (!in_array($taxonomy, $allowed, true)) {
            return ['success' => false, 'error' => "Taxonomie '{$taxonomy}' nicht erlaubt"];
        }
        ...
        switch ($action) {
            case 'list':
                ...
                return ['success' => true, 'terms' => array_map(fn($t) => [
                    'id' => $t->term_id,
                    'name' => $t->name,
                    'slug' => $t->slug,
                    'count' => $t->count,
            ...
            case 'create':
                $result = wp_insert_term((string) ($params['name'] ?? ''), $taxonomy, [
            ...
            case 'update':
                if (empty($params['term_id'])) {
                    return ['success' => false, 'error' => 'Erforderlich: term_id'];
                }
            ...
            case 'assign':
                $postIds = $params['post_ids'] ?? (empty($params['post_id']) ? [] : [(int) $params['post_id']]);
                $termIds = is_array($params['term_id'] ?? null) ? array_map('intval', $params['term_id']) : [(int) ($params['term_id'] ?? 0)];
                if (empty($postIds)) {
                    return ['success' => false, 'error' => 'Erforderlich: post_id oder post_ids'];
                }
                foreach ($postIds as $pid) {
                    wp_set_object_terms($pid, $termIds, $taxonomy, false);
                }
                return ['success' => true, 'assigned_to' => $postIds, 'terms' => $termIds];
    ...
    private function handleMenu(string $action, array $params): array {
        if (!current_user_can('edit_theme_options')) {
            return ['success' => false, 'error' => 'Keine Berechtigung für Menü-Verwaltung'];
        }

        switch ($action) {
            case 'create':
                $result = wp_create_nav_menu((string) ($params['name'] ?? 'New Menu'));
                if (is_wp_error($result)) {
                    return ['success' => false, 'error' => $result->get_error_message()];
                }
            ...
            case 'list':
                ...
                return ['success' => true, 'menus' => array_map(fn($m) => [
                    'id' => $m->term_id,
                    'name' => $m->name,
                    'slug' => $m->slug,
                    'count' => $m->count,
            ...
            case 'add_item':
                if (empty($params['menu_id'])) {
                    return ['success' => false, 'error' => 'Erforderlich: menu_id'];
                }
                $item = $params['item'] ?? [];
                $itemData = [
                ...
                $result = wp_update_nav_menu_item((int) $params['menu_id'], 0, $itemData);
                if (is_wp_error($result)) {
                    return ['success' => false, 'error' => $result->get_error_message()];
                }
            ...
            case 'remove_item':
                if (empty($params['item_id'])) {
                    return ['success' => false, 'error' => 'Erforderlich: item_id'];
                }
                wp_delete_post((int) $params['item_id'], true);
                return ['success' => true, 'removed' => true];

            default:
                return ['success' => false, 'error' => "Unbekannte Menü-Aktion: {$action}"];
        }
    }

    private function handleCron(string $action, array $params): array {
        if (!current_user_can('manage_options')) {
            return ['success' => false, 'error' => 'Keine Berechtigung für Cron-Verwaltung'];
        }
        ...
        switch ($action) {
            ...
            case 'schedule':
                ...
                $args = $params['args'] ?? [];
                if ($hook === '') {
                    return ['success' => false, 'error' => 'Erforderlich: hook'];
                }
                if (!wp_next_scheduled($hook, $args)) {
                    wp_schedule_event(time(), $schedule, $hook, $args);
                }
                return ['success' => true, 'hook' => $hook, 'scheduled' => true];
            ...
            case 'unschedule':
                ...
                return ['success' => true, 'hook' => $hook, 'unscheduled' => (bool) $timestamp];

            default:
                return ['success' => false, 'error' => "Unbekannte Cron-Aktion: {$action}"];
        }
    }

    private function handleMedia(string $action, array $params): array {
        if (!current_user_can('upload_files')) {
            return ['success' => false, 'error' => 'Keine Berechtigung für Medien-Upload'];
        }

        if ($action !== 'upload') {
            return ['success' => false, 'error' => "Medien-Aktion '{$action}' nicht unterstützt. Nutze 'upload'."];
        }

        if (!empty($params['file_url'])) {
            return $this->uploadFromUrl((string) $params['file_url'], $params['filename'] ?? null);
        }

        if (!empty($params['file_data'])) {
            return $this->uploadFromBase64((string) $params['file_data'], (string) ($params['filename'] ?? 'upload.bin'));
        }

        return ['success' => false, 'error' => 'Erforderlich: file_url oder file_data'];
    }
    ...
    private function uploadFromUrl(string $url, ?string $filename): array {
        ...
        if (is_wp_error($tmpFile)) {
            return ['success' => false, 'error' => $tmpFile->get_error_message()];
        }

        $fileArr = [
            'name' => $filename ?? basename(parse_url($url, PHP_URL_PATH) ?: 'download'),
            'tmp_name' => $tmpFile,
            'error' => 0,
    ...
    private function handleMeta(string $action, array $params): array {
        ...
        switch ($action) {
            case 'get':
                ...
                return ['success' => true, 'post_id' => $postId, 'key' => $key, 'value' => $value];
            ...
            case 'set':
                ...
                return ['success' => true, 'post_id' => $postId, 'key' => $key, 'action' => 'set'];
            ...
            case 'delete':
                ...
                return ['success' => true, 'post_id' => $postId, 'key' => $key, 'action' => 'delete'];

            default:
                return ['success' => false, 'error' => "Unbekannte Meta-Aktion: {$action}"];
        }
    }
}
