...
class GrepTool extends AbstractTool
{
    ...
    public function getParameters(): array
    {
        return [
            ...
            "directory" => [
                "type" => "string",
                "description" =>
                    "Verzeichnis für Dateisuche (absolut oder relativ zu wp-content/)",
            ],
            "post_type" => [
                "type" => "string",
                "description" =>
                    "Post-Type für Inhaltssuche (z.B. post, page, product)",
            ],
            "field" => [
                "type" => "string",
    ...
    public function execute(array $params): array
    {
        ...
        if (!empty($params["post_type"])) {
            return $this->grepPosts(
                $pattern,
                (string) $params["post_type"],
                (string) ($params["field"] ?? "both"),
                $regex,
                $caseSensitive,
                $limit,
            );
        ...
        return [
            "success" => false,
            "error" => "Erforderlich: directory ODER post_type",
        ];
    }
    ...
    private function grepFiles(
        ...
        if ($resolved === false) {
            return [
                "success" => false,
                "error" => "Verzeichnis nicht gefunden: {$directory}",
            ];
        }
        ...
        if (!$inAllowed) {
            return [
                "success" => false,
                "error" => "Pfad außerhalb erlaubter Verzeichnisse",
            ];
        }
