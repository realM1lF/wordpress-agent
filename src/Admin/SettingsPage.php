...
class SettingsPage
{
    ...
    public function __construct()
    {
        ...
        add_action("wp_ajax_levi_repair_database", [
            $this,
            "ajaxRepairDatabase",
        ]);
        add_action("wp_ajax_levi_run_state_snapshot", [
            $this,
            "ajaxRunStateSnapshot",
        ]);
        add_action("wp_ajax_levi_clear_audit_log", [
            $this,
            "ajaxClearAuditLog",
        ]);
        add_action("wp_ajax_levi_run_cron_task", [$this, "ajaxRunCronTask"]);
        add_action("wp_ajax_levi_toggle_cron_task", [
            $this,
            "ajaxToggleCronTask",
        ]);
        add_action("wp_ajax_levi_delete_cron_task", [
            $this,
            "ajaxDeleteCronTask",
        ]);
        add_action("wp_ajax_levi_toggle_cron_email", [
            $this,
            "ajaxToggleCronEmail",
        ]);
    ...
    public function enqueueAssets(): void
    {
        ...
        wp_enqueue_style(
            ...
            [],
            file_exists($settingsCss)
                ? (string) filemtime($settingsCss)
                : LEVI_AGENT_VERSION,
        );
        ...
        wp_enqueue_script(
            ...
            ["jquery"],
            file_exists($settingsJs)
                ? (string) filemtime($settingsJs)
                : LEVI_AGENT_VERSION,
            true,
        );
        ...
        wp_localize_script("levi-agent-settings", "leviSettings", [
            ...
            "i18n" => [
                ...
                "connectionError" => $this->tr(
                    "Connection error",
                    "Verbindungsfehler",
                ),
                "failed" => $this->tr("Failed", "Fehlgeschlagen"),
                "reloading" => $this->tr("Syncing…", "Synchronisiere..."),
                "confirmDisconnect" => $this->tr(
                    "Disconnect from OpenRouter? You will need to reconnect or enter an API key manually.",
                    "Von OpenRouter trennen? Du musst dich danach erneut verbinden oder einen API-Key manuell eingeben.",
                ),
                "fetchDocsConfirm" => $this->tr(
                    "Fetch latest docs from WooCommerce, WordPress & Elementor and sync? This may take several minutes.",
                    "Aktuelle Docs von WooCommerce, WordPress & Elementor abrufen und synchronisieren? Das kann einige Minuten dauern.",
                ),
                "fetchingDocs" => $this->tr(
                    "Fetching docs & syncing…",
                    "Docs werden abgerufen & synchronisiert...",
                ),
                "error" => $this->tr("Error", "Fehler"),
                "done" => $this->tr("Done", "Fertig"),
                "repairing" => $this->tr("Repairing…", "Repariere..."),
                "saving" => $this->tr("Saving…", "Speichere..."),
                "clearAuditConfirm" => $this->tr(
                    "Delete all audit log entries now?",
                    "Alle Audit-Log-Eintraege jetzt loeschen?",
                ),
    ...
    private function deriveThoroughness(
        int $historyLimit,
        array $tuningMode,
    ): string {
        if (
            isset($tuningMode["thoroughness"]) &&
            in_array(
                $tuningMode["thoroughness"],
                ["low", "balanced", "high"],
                true,
            )
        ) {
    ...
    {
        register_setting("levi_agent_settings_group", $this->optionName, [
            $this,
            "sanitizeSettings",
        ]);
    }
    ...
    public function sanitizeSettings(array $input): array
    {
        ...
        if (array_key_exists("ai_provider", $input)) {
            ...
            $sanitized["ai_provider"] = isset($providers[$provider])
                ? $provider
                : "openrouter";
        }
        ...
        if (array_key_exists("compact_model", $input)) {
            $sanitized["compact_model"] = sanitize_text_field(
                (string) ($input["compact_model"] ?? ""),
            );
        }
        if (array_key_exists("summary_model", $input)) {
            $sanitized["summary_model"] = sanitize_text_field(
                (string) ($input["summary_model"] ?? ""),
            );
        }

        // --- Numeric settings (safety / advanced tabs) ---
        if (array_key_exists("rate_limit", $input)) {
            $sanitized["rate_limit"] = max(
                1,
                min(1000, absint($input["rate_limit"])),
            );
        }
        if (array_key_exists("max_tokens", $input)) {
            $sanitized["max_tokens"] = max(
                1,
                min(131072, absint($input["max_tokens"])),
            );
        }
        ...
        if (array_key_exists("php_time_limit", $input)) {
            $sanitized["php_time_limit"] = max(
                0,
                absint($input["php_time_limit"]),
            );
        }
        ...
        if (in_array($thoroughness, ["low", "balanced", "high"], true)) {
            ...
            $sanitized["history_context_limit"] = max(
                10,
                min(200, absint($input["history_context_limit"])),
            );
        }
        ...
        if (array_key_exists("use_generic_tools", $input)) {
            $sanitized["use_generic_tools"] = (int) (bool) $input[
                "use_generic_tools"
            ];
        } else {
            $sanitized["use_generic_tools"] = 0;
        }
        ...
        if (in_array($speedMode, ["fast", "balanced", "careful"], true)) {
            ...
            $sanitized["max_tool_iterations"] = max(
                4,
                min(30, absint($input["max_tool_iterations"])),
            );
        }

        if ($thoroughness !== "" || $safetyMode !== "" || $speedMode !== "") {
            update_option("levi_setup_tuning_mode", [
                "thoroughness" => in_array(
                    $thoroughness,
                    ["low", "balanced", "high"],
                    true,
                )
                    ? $thoroughness
                    : "custom",
                "safety" => in_array($safetyMode, ["strict", "standard"], true)
                    ? $safetyMode
                    : "custom",
                "speed" => in_array(
                    $speedMode,
        ...
        if (array_key_exists("tool_profile", $input)) {
            ...
            $sanitized["tool_profile"] = in_array(
                $profileCandidate,
                \Levi\Agent\AI\Tools\Registry::VALID_PROFILES,
                true,
            )
                ? $profileCandidate
                : "standard";
        ...
        if (array_key_exists("allowed_plugin_slugs_manual", $input)) {
            $raw = sanitize_textarea_field(
                (string) $input["allowed_plugin_slugs_manual"],
            );
        ...
        if (array_key_exists("memory_identity_k", $input)) {
            $sanitized["memory_identity_k"] = max(
                1,
                min(20, absint($input["memory_identity_k"])),
            );
        }
        if (array_key_exists("memory_reference_k", $input)) {
            $sanitized["memory_reference_k"] = max(
                1,
                min(20, absint($input["memory_reference_k"])),
            );
        }
        if (array_key_exists("memory_min_similarity", $input)) {
            $sanitized["memory_min_similarity"] = max(
                0.0,
                min(1.0, (float) $input["memory_min_similarity"]),
            );
        }

        // --- Safety tab booleans (hidden input ensures key is present when tab is rendered) ---
        if (array_key_exists("pii_redaction", $input)) {
            $sanitized["pii_redaction"] = !empty($input["pii_redaction"])
                ? 1
                : 0;
        }
        if (array_key_exists("blocked_post_types", $input)) {
            $sanitized["blocked_post_types"] = sanitize_textarea_field(
                $input["blocked_post_types"],
            );
        }
    ...
    public function renderPage(): void
    {
        ...
        $apiKeyStatus = $this->getApiKeyForProvider($provider)
            ? "connected"
            : "disconnected";
        ...
        $tabs = [
            "general" => [
                ...
                "label" => $this->tr("Dashboard", "Dashboard"),
            ],
            "ai-provider" => [
                "icon" => "dashicons-cloud",
                "label" => $this->tr("AI Provider", "KI-Anbieter"),
            ],
            "memory" => [
                "icon" => "dashicons-database",
                "label" => $this->tr("Memory", "Memory"),
            ],
            "safety" => [
                "icon" => "dashicons-shield",
                "label" => $this->tr("Limits & Safety", "Limits & Sicherheit"),
            ],
            "advanced" => [
                "icon" => "dashicons-admin-tools",
                "label" => $this->tr("Advanced", "Erweitert"),
            ],
            "cron-tasks" => [
                "icon" => "dashicons-clock",
                "label" => $this->tr("Scheduled Tasks", "Geplante Aufgaben"),
            ],
        ];
        ?>
                ...
                <div class="levi-header-content">
                    <div class="levi-logo">
                        ...
                        <div class="levi-logo-text">
                            ...
                            <span class="levi-version">v<?php echo esc_html(
                                LEVI_AGENT_VERSION,
                            ); ?></span>
                        </div>
                    </div>
                    <div class="levi-connection-status levi-status-<?php echo esc_attr(
                        $apiKeyStatus,
                    ); ?>">
                        <span class="levi-status-dot"></span>
                        <span class="levi-status-text">
                            <?php echo $apiKeyStatus === "connected"
                                ? esc_html($this->tr("Connected", "Verbunden"))
                                : esc_html(
                                    $this->tr(
                                        "Not Connected",
                                        "Nicht verbunden",
                ...
                <div class="levi-nav-tabs">
                    <?php foreach ($tabs as $tabId => $tabData):
                        ...
                        $tabUrl = add_query_arg(
                            ["page" => $this->pageSlug, "tab" => $tabId],
                            admin_url("admin.php"),
                        );
                        ?>
                        <a href="<?php echo esc_url($tabUrl); ?>"
                           class="levi-nav-tab <?php echo $isActive
                               ? "levi-nav-tab-active"
                               : ""; ?>">
        ...
        <div class="levi-settings-section">
            <div class="levi-section-header">
                <h2><?php echo esc_html(
                    $this->tr("Welcome to Levi", "Willkommen bei Levi"),
                ); ?></h2>
                <p><?php echo esc_html(
                    $this->tr(
                        "Your AI-powered WordPress assistant. Configure the basic settings below.",
                        "Dein KI-Assistent fuer WordPress. Konfiguriere hier die wichtigsten Grundeinstellungen.",
                    ),
                ); ?></p>
            </div>
            ...
            <div class="levi-cards-grid">
                ...
                <div class="levi-card levi-card-featured">
                    ...
                    <h3><?php echo esc_html(
                        $this->tr("Quick Start", "Schnellstart"),
                    ); ?></h3>
                    <p><?php echo esc_html(
                        $this->tr(
                            "Levi is ready to help you manage your WordPress site. Open the chat widget in the bottom right corner to get started.",
                            "Levi hilft dir bei deiner WordPress-Seite. Oeffne unten rechts das Chat-Widget, um zu starten.",
                        ),
                    ); ?></p>
                    <a href="#" class="levi-btn levi-btn-secondary" onclick="document.getElementById('levi-chat-toggle').click(); return false;">
                        <?php echo esc_html(
                            $this->tr("Open Chat", "Chat oeffnen"),
                        ); ?>
                    </a>
                </div>
                ...
                <div class="levi-card">
                    <div class="levi-card-header">
                        ...
                        <h3><?php echo esc_html(
                            $this->tr("Connection Status", "Verbindungsstatus"),
                        ); ?></h3>
                    </div>
                    <div class="levi-card-content">
                        ...
                        $provider = $this->getProvider();
                        $providerLabel =
                            $this->getProviderLabels()[$provider] ??
                            ucfirst($provider);
                        $isConfigured = $this->getApiKeyForProvider($provider)
                            ? true
                            : false;
                        ?>
                        <div class="levi-status-row">
                            <span class="levi-status-label"><?php echo esc_html(
                                $this->tr("Provider", "Anbieter"),
                            ); ?></span>
                            <span class="levi-status-value"><?php echo esc_html(
                                $providerLabel,
                            ); ?></span>
                        </div>
                        <div class="levi-status-row">
                            <span class="levi-status-label"><?php echo esc_html(
                                $this->tr("Model", "Modell"),
                            ); ?></span>
                            <span class="levi-status-value"><?php echo esc_html(
                                $this->getAltModelForProvider($provider),
                            ); ?></span>
                        </div>
                        <div class="levi-status-row">
                            <span class="levi-status-label"><?php echo esc_html(
                            ...
                            <span class="levi-status-badge <?php echo $isConfigured
                                ? "levi-badge-success"
                                : "levi-badge-warning"; ?>">
                                <?php echo $isConfigured
                                    ? esc_html(
                                        $this->tr("Connected", "Verbunden"),
                                    )
                                    : esc_html(
                                        $this->tr(
                                            "Setup Required",
                ...
                <div class="levi-card">
                    <div class="levi-card-header">
                        ...
                        <h3><?php echo esc_html(
                            $this->tr("Memory Stats", "Memory-Statistik"),
                        ); ?></h3>
                    </div>
                    <div class="levi-card-content">
                        <?php
                        ...
                        <div class="levi-stat-item">
                            <span class="levi-stat-value"><?php echo number_format(
                                $stats["identity_files"] ?? 0,
                            ); ?></span>
                            <span class="levi-stat-label"><?php _e(
                                "Identity",
                                "levi-agent",
                            ); ?></span>
                            <?php
                            $identityNames =
                                $stats["identity_file_names"] ?? [];
                            if (!empty($identityNames)): ?>
                                <p class="levi-stat-files"><?php echo esc_html(
                                    implode(", ", $identityNames),
                                ); ?></p>
                            <?php endif;
                            ?>
                        </div>
                        <div class="levi-stat-item">
                            ...
                            <?php
                            $referenceNames =
                                $stats["reference_file_names"] ?? [];
                            if (!empty($referenceNames)): ?>
                                <p class="levi-stat-files"><?php echo esc_html(
                                    implode(", ", $referenceNames),
                                ); ?></p>
                            <?php endif;
                            ?>
                        </div>
                ...
                <div class="levi-card">
                    <div class="levi-card-header">
                        ...
                        <h3><?php echo esc_html(
                            $this->tr("Plan & Setup", "Plan & Einrichtung"),
                        ); ?></h3>
                    </div>
                    <div class="levi-card-content">
                        <?php
                        ...
                        $setupDone =
                            (int) get_option("levi_setup_completed", 0) === 1;
                        ?>
                        <div class="levi-status-row">
                            <span class="levi-status-label"><?php echo esc_html(
                                $this->tr("Active Plan", "Aktiver Plan"),
                            ); ?></span>
                            <span class="levi-status-value">
                                <?php if ($planTier === "pro"): ?>
                                    <span class="levi-badge levi-badge-success">Pro</span>
                                <?php else: ?>
                                    <span class="levi-badge levi-badge-warning"><?php echo esc_html(
                                        $this->tr("Not set", "Nicht gesetzt"),
                                    ); ?></span>
                                <?php endif; ?>
                            </span>
                        </div>
                        <div class="levi-status-row">
                            ...
                            <span class="levi-status-value">
                                <?php echo $setupDone
                                    ? esc_html(
                                        $this->tr("Completed", "Abgeschlossen"),
                                    )
                                    : esc_html(
                                        $this->tr("Pending", "Ausstehend"),
                                    ); ?>
                            </span>
                        </div>
                    </div>
                    <div class="levi-card-footer">
                        <a href="<?php echo esc_url(
                            admin_url(
                                "admin.php?page=levi-agent-setup-wizard&step=1",
                            ),
                        ); ?>" class="levi-btn levi-btn-small levi-btn-secondary">
    ...
    private function renderAiProviderTab(array $settings): void
    {
        ...
        <div class="levi-settings-section">
            <div class="levi-section-header">
                <h2><?php echo esc_html(
                    $this->tr("AI Provider", "KI-Anbieter"),
                ); ?></h2>
                <p><?php echo esc_html(
                    $this->tr(
                        "Choose your AI provider and configure the connection.",
                        "Waehle deinen KI-Anbieter und konfiguriere die Verbindung.",
                    ),
                ); ?></p>
            </div>
            ...
            <div class="levi-form-card">
                ...
                <div class="levi-form-group">
                    <select name="<?php echo esc_attr(
                        $this->optionName,
                    ); ?>[ai_provider]" class="levi-form-select" id="levi-provider-select">
                        <?php foreach ($providers as $pId => $pLabel): ?>
                            <option value="<?php echo esc_attr(
                                $pId,
                            ); ?>" <?php selected($provider, $pId); ?>>
                                <?php echo esc_html($pLabel); ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                    <p class="levi-form-help levi-provider-hint" data-provider="openrouter"<?php echo $provider !==
                    "openrouter"
                        ? ' style="display:none;"'
                        : ""; ?>>
                        <?php echo esc_html(
                    ...
                    <p class="levi-form-help levi-provider-hint" data-provider="anthropic"<?php echo $provider !==
                    "anthropic"
                        ? ' style="display:none;"'
                        : ""; ?>>
                        <?php echo esc_html(
                    ...
                    <p class="levi-form-help levi-provider-hint" data-provider="openai"<?php echo $provider !==
                    "openai"
                        ? ' style="display:none;"'
                        : ""; ?>>
                        <?php echo esc_html(
            ...
            <div class="levi-form-card">
                <h3><?php echo esc_html(
                    $this->tr("Authentication", "Authentifizierung"),
                ); ?></h3>
                ...
                <?php foreach ($allProviders as $p):
                    ...
                    ?>
                    <div class="levi-provider-auth-section" data-provider="<?php echo esc_attr(
                        $p,
                    ); ?>"<?php echo $p !== $provider
    ? ' style="display:none;"'
    : ""; ?>>
                        <?php if ($p === "openrouter"): ?>
                            <?php $this->renderOpenRouterAuth(
                                $settings,
                                $pHasKey,
                                $pKeyField,
                            ); ?>
                        <?php else: ?>
                            <?php $this->renderDirectApiKeyAuth(
                                $p,
                                $pHasKey,
                                $pKeyField,
                            ); ?>
                ...
                <?php if (!empty($_GET["oauth_success"])): ?>
                    <div class="levi-notice levi-notice-success levi-provider-auth-section" data-provider="openrouter" style="margin-top: 12px; padding: 10px 14px; background: #ecf7ed; border-left: 4px solid #46b450; border-radius: 4px;<?php echo $provider !==
                    "openrouter"
                        ? " display:none;"
                        : ""; ?>">
                        <?php echo esc_html(
                ...
                <?php if (!empty($_GET["oauth_error"])): ?>
                    <div class="levi-notice levi-notice-error levi-provider-auth-section" data-provider="openrouter" style="margin-top: 12px; padding: 10px 14px; background: #fbeaea; border-left: 4px solid #dc3232; border-radius: 4px;<?php echo $provider !==
                    "openrouter"
                        ? " display:none;"
                        : ""; ?>">
                        <?php
                        ...
                        echo esc_html(
                            $errorMessages[$errorCode] ??
                                $this->tr(
                                    "OAuth error. Please try again.",
                                    "OAuth-Fehler. Bitte erneut versuchen.",
                                ),
                        );
                        $details = isset($_GET["oauth_details"])
                            ? sanitize_text_field(
                                wp_unslash($_GET["oauth_details"]),
                            )
                            : "";
                        if ($details !== "") {
                ...
                <div class="levi-form-actions-inline" style="margin-top: 16px;">
                    <button type="button" id="levi-test-connection" class="levi-btn levi-btn-secondary">
                        ...
                        <?php echo esc_html(
                            $this->tr("Test Connection", "Verbindung testen"),
                        ); ?>
                    </button>
            ...
            <div class="levi-form-card">
                ...
                <?php foreach ($allProviders as $p):
                    ...
                    ?>
                    <div class="levi-provider-model-section" data-provider="<?php echo esc_attr(
                        $p,
                    ); ?>"<?php echo $p !== $provider
    ? ' style="display:none;"'
    : ""; ?>>
    ...
    $pModelKey,
); ?>]" class="levi-form-select">
                                <?php foreach (
                                    $pModels
                                    as $modelId => $modelLabel
                                ): ?>
                                    <option value="<?php echo esc_attr(
                                        $modelId,
                                    ); ?>" <?php selected(
    ...
    private function renderOpenRouterAuth(
        array $settings,
        bool $hasKey,
        string $keyField,
    ): void {
        ...
        if ($isOAuth): ?>
            <div class="levi-oauth-connected">
                <div class="levi-oauth-status">
                    ...
                    <div>
                        <strong><?php echo esc_html(
                            $this->tr(
                                "Connected via OpenRouter OAuth",
                                "Verbunden ueber OpenRouter OAuth",
                            ),
                        ); ?></strong>
                ...
                <button type="button" id="levi-oauth-disconnect" class="levi-btn levi-btn-danger levi-btn-small" style="margin-top: 12px;">
                    ...
                    <?php echo esc_html(
                        $this->tr("Disconnect", "Verbindung trennen"),
                    ); ?>
                </button>
            </div>
        <?php else:$oauthUrl = $oauth->getAuthUrl("settings"); ?>
            <div class="levi-oauth-connect">
                ...
                <div class="levi-cost-hint" style="margin-bottom: 16px; padding: 10px 14px; background: rgba(124, 58, 237, 0.08); border-left: 4px solid var(--levi-accent, #7c3aed); border-radius: 4px; font-size: 13px; color: var(--levi-text-secondary, #94a3b8);">
                    <strong style="color: var(--levi-text-primary, #f1f5f9);"><?php echo esc_html(
                        $this->tr("Typical costs:", "Typische Kosten:"),
                    ); ?></strong>
                    <?php echo esc_html(
                        $this->tr(
                    ...
                    <div style="margin-top: 8px; padding-top: 8px; border-top: 1px solid rgba(124, 58, 237, 0.15);">
                        <strong style="color: var(--levi-text-primary, #f1f5f9); font-size: 14px;"><?php echo esc_html(
                            $this->tr(
                                '$5 credit is plenty to get started!',
                                '5$ Guthaben reichen fuer den Einstieg locker aus!',
                            ),
                        ); ?></strong>
                    </div>
                </div>
                <a href="<?php echo esc_url(
                    $oauthUrl,
                ); ?>" class="levi-btn levi-btn-primary" style="display: inline-flex; align-items: center; gap: 8px; text-decoration: none;">
            ...
            <div class="levi-oauth-divider" style="margin: 20px 0; display: flex; align-items: center; gap: 12px;">
                ...
                <span style="color: #999; font-size: 13px;"><?php echo esc_html(
                    $this->tr("or enter manually", "oder manuell eingeben"),
                ); ?></span>
                <hr style="flex: 1; border: none; border-top: 1px solid #ddd;">
            </div>
            ...
            <div class="levi-form-group">
                <label class="levi-form-label">
                    <?php echo esc_html(
                        $this->tr("API Key", "API-Schluessel"),
                    ); ?>
                    <?php if ($hasKey): ?>
                        <span class="levi-badge levi-badge-success"><?php echo esc_html(
                            $this->tr("Configured", "Konfiguriert"),
                        ); ?></span>
                    <?php endif; ?>
                </label>
                <input type="password"
                    ...
                    <?php if ($hasKey): ?>
                        <?php echo esc_html(
                            $this->tr(
                                ...
                                "API-Schluessel ist gespeichert. Gib einen neuen ein, um ihn zu ersetzen.",
                            ),
                        ); ?>
                    <?php else: ?>
                        <?php echo esc_html(
    ...
    private function renderDirectApiKeyAuth(
        string $provider,
        bool $hasKey,
        string $keyField,
    ): void {
        ...
        <div class="levi-form-group">
            ...
            <input type="password"
                   ...
                   placeholder="<?php echo $hasKey
                       ? "••••••••••••••••••••"
                       : esc_attr($providerConfig["placeholder"]); ?>">
            <p class="levi-form-help">
                <?php if ($hasKey): ?>
                    <?php echo esc_html(
                        $this->tr(
                ...
                <?php else: ?>
                    <?php printf(
                        esc_html(
                            $this->tr(
                                "Get your key at %s",
                                "Hole deinen Key auf %s",
                            ),
                        ),
                        '<a href="' .
                            esc_url($providerConfig["url_full"]) .
                            '" target="_blank" rel="noopener">' .
                            esc_html($providerConfig["url"]) .
                            "</a>",
                    ); ?>
                <?php endif; ?>
            </p>
    ...
    private function renderModelComparisonTable(string $provider): void
    {
        ...
        <div class="levi-model-info">
            <h4><?php echo esc_html(
                $this->tr("Model Comparison", "Modell-Vergleich"),
            ); ?></h4>
                ...
                <table class="levi-data-table">
                    <thead>
                        <tr>
                            ...
                            <th><?php echo esc_html(
                                $this->tr("Context", "Kontext"),
                            ); ?></th>
                            <th><?php echo esc_html(
                                $this->tr("Best For", "Geeignet fuer"),
                            ); ?></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($tables as $row): ?>
                            <tr<?php echo !empty($row["featured"])
                                ? ' class="levi-row-featured"'
                                : ""; ?>>
                                <td><?php echo !empty($row["featured"])
                                    ? '<span class="levi-model-name">' .
                                        esc_html($row["name"]) .
                                        "</span>"
                                    : esc_html($row["name"]); ?></td>
                                <td><span class="levi-price"><?php echo esc_html(
                                    $row["price"],
                                ); ?></span></td>
        ...
        return match ($provider) {
            "anthropic" => [
                [
                    ...
                    "context" => "1M",
                    "best_for" => $this->tr(
                        "Most intelligent, agents & coding",
                        "Intelligentestes Modell, Agenten & Coding",
                    ),
                ],
                [
                    ...
                    "best_for" => $this->tr(
                        ...
                        "Beste Balance aus Speed & Intelligenz",
                    ),
                    "featured" => true,
                ],
                [
                    ...
                    "best_for" => $this->tr(
                        ...
                        "Schnellstes Modell, nahe Spitzenklasse",
                    ),
                ],
                [
                    ...
                    "best_for" => $this->tr(
                        ...
                        "Bewaehrtes, zuverlaessiges Modell",
                    ),
                ],
            ],
            "openai" => [
                [
                    ...
                    "context" => "1M",
                    "best_for" => $this->tr(
                        "Flagship, complex reasoning & coding",
                        "Flagship, komplexes Reasoning & Coding",
                    ),
                    "featured" => true,
                ],
                [
                    ...
                    "best_for" => $this->tr(
                        ...
                        "Schnell & guenstig, ideal fuer die meisten Aufgaben",
                    ),
                ],
                [
                    "name" => "GPT-4o",
            ...
            default => [
                ...
                [
                    ...
                    "context" => "400K",
                    "best_for" => $this->tr(
                        "Max coding quality, large context",
                        "Max. Coding-Qualitaet, grosser Kontext",
                    ),
                ],
                [
                    "name" => "Claude Opus 4.6",
                    ...
                    "best_for" => $this->tr(
                        ...
                        "Intelligentestes verfuegbares Modell",
                    ),
                ],
                [
                    "name" => "Claude Sonnet 4.6",
                    ...
                    "context" => "1M",
                    "best_for" => $this->tr(
                        "Fast flagship Claude",
                        "Schneller Flagship Claude",
                    ),
                ],
                [
                ...
                [
                    ...
                    "price" => '$0.15 / $0.60',
                    "context" => "128K",
                    "best_for" => $this->tr("Budget option", "Budget-Option"),
                ],
            ],
        };
    }
    ...
    private function renderMemoryTab(array $settings): void
    {
        ...
        <div class="levi-settings-section">
            <div class="levi-section-header">
                <h2><span class="dashicons dashicons-database"></span> <?php echo esc_html(
                    $this->tr("Memory Configuration", "Memory konfigurieren"),
                ); ?></h2>
                <p><?php echo esc_html(
                    $this->tr(
                        "Configure how Levi remembers and retrieves information.",
                        "Steuere, wie Levi Informationen merkt und wiederfindet.",
                    ),
                ); ?></p>
            </div>
            ...
            <div class="levi-cards-grid levi-cards-2col">
                ...
                <div class="levi-form-card">
                    ...
                    <p class="levi-form-description">
                        <?php echo esc_html(
                            $this->tr(
                                "Control how many memories are retrieved for each query type.",
                    ...
                    <div class="levi-form-row">
                        <div class="levi-form-group">
                            <label class="levi-form-label"><?php echo esc_html(
                                $this->tr(
                                    "Identity Memories",
                                    "Identity-Memories",
                                ),
                            ...
                            <input type="number"
                                   ...
                                   value="<?php echo esc_attr(
                                       $settings["memory_identity_k"],
                                   ); ?>"
                                   min="1" max="20" class="levi-form-input levi-input-small">
                            <p class="levi-form-help levi-hint">
                                <?php echo esc_html(
                                    $this->tr(
                                        "Controls how many identity entries are loaded (persona, role, style). Higher values add more personal context but can dilute focus. Lower values are stricter and faster.",
                                        "Steuert, wie viele Identity-Eintraege (Rolle, Persona, Stil) geladen werden. Hoehere Werte geben mehr persoenlichen Kontext, koennen aber den Fokus verwaessern. Niedrigere Werte sind strenger und schneller.",
                                    ),
                                ); ?>
                            </p>
                        </div>
                        <div class="levi-form-group">
                            ...
                            <input type="number"
                                   ...
                                   value="<?php echo esc_attr(
                                       $settings["memory_reference_k"],
                                   ); ?>"
                                   min="1" max="20" class="levi-form-input levi-input-small">
                            <p class="levi-form-help levi-hint">
                                <?php echo esc_html(
                                    $this->tr(
                                        "Controls how many knowledge/reference entries are loaded (docs, rules, facts). Higher values increase coverage but may add noise. Lower values keep answers tighter and more selective.",
                                        "Steuert, wie viele Wissens-/Referenz-Eintraege (Dokus, Regeln, Fakten) geladen werden. Hoehere Werte erhoehen die Abdeckung, koennen aber mehr Rauschen bringen. Niedrigere Werte machen Antworten fokussierter und selektiver.",
                                    ),
                                ); ?>
                            </p>
                        </div>
                    ...
                    <div class="levi-form-row">
                        <div class="levi-form-group">
                            ...
                            <p class="levi-form-help levi-hint">
                                <?php echo esc_html(
                                    $this->tr(
                                        "Similarity threshold from 0 to 1. Higher values require closer semantic matches (fewer, more precise results). Lower values allow broader matches (more results, potentially less relevant).",
                                        "Aehnlichkeitsschwelle von 0 bis 1. Hoehere Werte verlangen engere semantische Treffer (weniger, aber praeziser). Niedrigere Werte erlauben breitere Treffer (mehr Ergebnisse, evtl. weniger relevant).",
                                    ),
                                ); ?>
                            </p>
                        </div>
                ...
                <div class="levi-form-card">
                    <h3><?php echo esc_html(
                        $this->tr("Memory Files", "Memory-Dateien"),
                    ); ?></h3>
                    ...
                    <?php if ($hasIdentityChanges || $hasReferenceChanges): ?>
                        <div id="levi-memory-changes-warning" class="levi-notice levi-notice-warning">
                            <p><strong><?php echo esc_html(
                                $this->tr(
                                    "Files have changed and need syncing:",
                                    "Dateien haben sich geaendert und muessen synchronisiert werden:",
                                ),
                            ); ?></strong></p>
                            <?php if ($hasIdentityChanges): ?>
                                <p><?php echo esc_html(
                                    $this->tr("Identity:", "Identity:"),
                                ); ?> <?php echo esc_html(
     implode(", ", $changes["identity"]),
 ); ?></p>
                            <?php endif; ?>
                            <?php if ($hasReferenceChanges): ?>
                                <p><?php echo esc_html(
                                    $this->tr("Reference:", "Referenz:"),
                                ); ?> <?php echo esc_html(
     implode(", ", $changes["reference"]),
 ); ?></p>
                            <?php endif; ?>
                        </div>
                    <?php endif; ?>
                    ...
                    <?php if (!empty($syncMeta)): ?>
                        <div class="levi-notice levi-notice-<?php echo ($syncMeta[
                            "status"
                        ] ??
                            "") ===
                        "synced"
                            ? "success"
                            : (($syncMeta["status"] ?? "") === "unchanged"
                                ? "info"
                                : "warning"); ?>" style="margin-bottom: 1rem;">
                            <p><strong><?php echo esc_html(
                                $this->tr("Last sync:", "Letzter Sync:"),
                            ); ?></strong> <?php echo esc_html(
    $syncMeta["synced_at"] ?? "—",
); ?> — <?php echo esc_html($syncMeta["status"] ?? "unknown"); ?></p>
                            <?php if (!empty($syncMeta["errors"])): ?>
                                <p class="levi-hint"><?php echo esc_html(
                                    implode(", ", $syncMeta["errors"]),
                                ); ?></p>
                            <?php endif; ?>
                        </div>
                    <?php endif; ?>
     ...
     $fetchMeta["fetched_at"],
 ); ?> (<?php echo esc_html($fetchMeta["status"] ?? "unknown"); ?>)
                        </p>
                    <?php else: ?>
                        <p class="levi-form-help levi-hint">
                            <?php echo esc_html(
                                $this->tr(
                                    'Docs are fetched automatically daily at 04:00. Click "Fetch Docs" to trigger manually.',
                                    'Docs werden taeglich um 04:00 Uhr automatisch abgerufen. Klicke "Docs abrufen" fuer manuellen Abruf.',
                                ),
                            ); ?>
                        </p>
                    <?php endif; ?>
                ...
                <div class="levi-form-card">
                    ...
                    <div class="levi-card-content">
                        ...
                        $snapshotStatus =
                            (string) ($snapshotMeta["status"] ?? "not_run");
                        $snapshotCapturedAt =
                            (string) ($snapshotMeta["captured_at"] ?? "-");
                        ?>
                        <div class="levi-status-row">
                            <span class="levi-status-label"><?php echo esc_html(
                                $this->tr("Last Run", "Letzter Lauf"),
                            ); ?></span>
                            <span class="levi-status-value"><?php echo esc_html(
                                $snapshotCapturedAt,
                            ); ?></span>
                        ...
                        <div class="levi-status-row">
                            ...
                            <span class="levi-status-badge levi-badge-<?php echo $snapshotStatus ===
                            "changed_stored"
                                ? "success"
                                : "neutral"; ?>">
                                <?php echo esc_html(
                                    $this->translateSnapshotStatus(
                                        $snapshotStatus,
                                    ),
                                ); ?>
                            </span>
                    ...
                    <p class="levi-form-help levi-hint">
                        <?php echo esc_html(
                            $this->tr(
                                "Hint: The daily snapshot indexes your WordPress state (plugins, themes, config) so Levi can answer questions about your site. Run manually here or wait for the scheduled task.",
                                "Hinweis: Der taegliche Snapshot indexiert deinen WordPress-Stand (Plugins, Themes, Konfiguration), damit Levi Fragen zur Seite beantworten kann. Hier manuell starten oder auf den geplanten Lauf warten.",
                            ),
                        ); ?>
                    </p>
                    <div class="levi-card-footer">
                        <button type="button" id="levi-run-state-snapshot" class="levi-btn levi-btn-small levi-btn-secondary">
                            <span class="dashicons dashicons-update"></span>
                            <?php echo esc_html(
                                $this->tr("Run Now", "Jetzt ausfuehren"),
                            ); ?>
                        </button>
    ...
    private function renderSafetyTab(array $settings): void
    {
        ...
        <div class="levi-settings-section">
            <div class="levi-section-header">
                <h2><?php echo esc_html(
                    $this->tr("Limits & Safety", "Limits & Sicherheit"),
                ); ?></h2>
                <p><?php echo esc_html(
                    $this->tr(
                        "Configure safety measures and usage limits.",
                        "Konfiguriere Sicherheitsmechanismen und Nutzungsgrenzen.",
                    ),
                ); ?></p>
            </div>
            ...
            <div class="levi-form-card" style="margin-bottom: 1.5rem;">
                <h3><?php echo esc_html(
                    $this->tr("Tool Profile", "Tool-Profil"),
                ); ?></h3>
                <p class="levi-form-description">
                    <?php echo esc_html(
                        $this->tr(
                ...
                <div class="levi-form-group">
                    ...
                    foreach ($profiles as $profileKey => $profileData): ?>
                        <label class="levi-radio-card <?php echo $currentProfile ===
                        $profileKey
                            ? "levi-radio-card-active"
                            : ""; ?>" style="display:flex; align-items:flex-start; gap:0.75rem; padding:0.75rem 1rem; border:1px solid <?php echo $currentProfile ===
$profileKey
    ? "#7c3aed"
    : "#374151"; ?>; border-radius:8px; margin-bottom:0.5rem; cursor:pointer; background:<?php echo $currentProfile ===
$profileKey
    ...
    : "transparent"; ?>;">
                            ...
                            <div>
                                ...
                                <p class="levi-form-help" style="margin:0.25rem 0 0;"><?php echo esc_html(
            ...
            <!-- Generic Tools Toggle (0.9.0) -->
            <div class="levi-form-card" style="margin-bottom: 1.5rem;">
                <h3><?php echo esc_html(
                    $this->tr(
                        "Generic Tools (Beta)",
                        "Generische Tools (Beta)",
                    ),
                ); ?></h3>
                <p class="levi-form-description">
                    <?php echo esc_html(
                        $this->tr(
                            "Enable 12 unified generic tools instead of 44+ specialized ones. Higher accuracy, lower token usage, simpler schemas.",
                            "Aktiviere 12 vereinheitlichte generische Tools statt 44+ spezialisierter. Hoehere Genauigkeit, niedrigerer Token-Verbrauch, einfachere Schemas.",
                        ),
                    ); ?>
                </p>
                <div class="levi-form-group">
                    <label style="display:flex; align-items:center; gap:0.5rem; cursor:pointer;">
                        <input type="checkbox"
                               ...
                               value="1"
                               <?php checked(
                                   !empty($settings["use_generic_tools"]),
                               ); ?>>
                        <span><?php echo esc_html(
                            $this->tr(
                                "Use generic tools (recommended for testing)",
                                "Generische Tools verwenden (empfohlen zum Testen)",
                            ),
                        ); ?></span>
                    </label>
                </div>
                <p class="levi-form-help levi-hint">
                    <?php echo esc_html(
                        $this->tr(
                            "Beta: Requires page reload after saving. Fallback to specialized tools available by disabling this option.",
                            "Beta: Erfordert Seiten-Neuladen nach dem Speichern. Fallback auf spezialisierte Tools moeglich durch Deaktivieren.",
                        ),
                    ); ?>
                </p>
            </div>
            ...
            <div class="levi-form-card" style="margin-bottom: 1.5rem;">
                ...
                <div class="levi-form-group">
                    <label class="levi-form-label"><?php echo esc_html(
                        $this->tr(
                            "Manual allowed plugin slugs",
                            "Manuell erlaubte Plugin-Slugs",
                        ),
                    ); ?></label>
                    ...
                    ><?php echo esc_textarea(
                        $settings["allowed_plugin_slugs_manual"] ?? "",
                    ); ?></textarea>
                    <p class="levi-form-help">
                        <?php echo esc_html(
                            $this->tr(
            ...
            <div class="levi-form-card" style="margin-bottom: 1.5rem;">
                <h3>🛡️ <?php echo esc_html(
                    $this->tr("Data Protection", "Datenschutz"),
                ); ?></h3>
                <p class="levi-form-description">
                    <?php echo esc_html(
                        $this->tr(
                ...
                <div class="levi-toggle-group" style="margin-bottom: 1rem;">
                    <label class="levi-toggle">
                        <input type="checkbox"
                               ...
                               value="1"
                               <?php checked(
                                   !isset($settings["pii_redaction"]) ||
                                       !empty($settings["pii_redaction"]),
                               ); ?>>
                        <span class="levi-toggle-slider"></span>
                        <span class="levi-toggle-label">
                            <?php echo esc_html(
                                $this->tr(
                                    "Enable PII redaction & post type / meta key blocking",
                                    "PII-Redaction & Post-Type- / Meta-Key-Blocking aktivieren",
                                ),
                            ); ?>
                        </span>
                    </label>
                ...
                <div class="levi-form-group">
                    <label class="levi-form-label"><?php echo esc_html(
                        $this->tr(
                            "Additional blocked post types",
                            "Zusaetzlich blockierte Post-Types",
                        ),
                    ); ?></label>
                    <textarea name="<?php echo esc_attr(
                        $this->optionName,
            ...
            <div class="levi-cards-grid levi-cards-2col">
                ...
                <div class="levi-form-card">
                    <h3><?php echo esc_html(
                        $this->tr("Rate Limiting", "Rate-Limit"),
                    ); ?></h3>
                    <div class="levi-form-group">
                        <label class="levi-form-label"><?php echo esc_html(
                            $this->tr(
                        ...
                        <p class="levi-form-help">
                            <?php echo esc_html(
                                $this->tr(
                                    "Maximum API requests per user per hour to control costs.",
                                    "Maximale API-Anfragen pro Benutzer und Stunde zur Kostenkontrolle.",
                                ),
                            ); ?>
                        </p>
                        <p class="levi-form-help levi-hint">
                            <?php echo esc_html(
                                $this->tr(
                                    "Hint: Lower values reduce API costs; increase if multiple editors use Levi frequently.",
                                    "Hinweis: Niedrige Werte senken Kosten. Erhoehen, wenn mehrere Redakteure Levi oft nutzen.",
                                ),
                            ); ?>
                        </p>
                    </div>
                ...
                <div class="levi-form-card">
                    <h3><?php echo esc_html(
                        $this->tr("Levi Behavior", "Levi-Verhalten"),
                    ); ?></h3>
                    ...
                    if (!is_array($tuningMode)) {
                        $tuningMode = [];
                    }
                    $curHistoryLimit =
                        (int) ($settings["history_context_limit"] ?? 20);
                    $curThoroughness = $this->deriveThoroughness(
                        $curHistoryLimit,
                        $tuningMode,
                    );
                    $curSafety = empty($settings["allow_destructive"])
                        ? "strict"
                        : "standard";
                    $curMaxIterations =
                        (int) ($settings["max_tool_iterations"] ?? 25);
                    $curSpeed = $this->deriveSpeed(
                        $curMaxIterations,
                        $tuningMode,
                    );
                    ?>
                    ...
                    <div class="levi-form-group">
                        <label class="levi-form-label" for="levi_thoroughness"><?php echo esc_html(
                            $this->tr(
                                "How much chat history should Levi consider?",
                                "Wie viel Chat-Verlauf soll Levi beruecksichtigen?",
                            ),
                        ); ?></label>
                        <div style="display: flex; gap: 0.75rem; align-items: center;">
                            <select id="levi_thoroughness" name="<?php echo esc_attr(
                                $this->optionName,
                            ); ?>[levi_thoroughness]" class="levi-form-select" style="flex: 1;">
                                <option value="low" <?php selected(
                                    $curThoroughness,
                                    "low",
                                ); ?>><?php echo esc_html($this->tr("Few (10 messages)", "Wenig (10 Nachrichten)")); ?></option>
                                <option value="balanced" <?php selected(
                                    $curThoroughness,
                                    "balanced",
                                ); ?>><?php echo esc_html($this->tr("Medium (20 messages, recommended)", "Mittel (20 Nachrichten, empfohlen)")); ?></option>
                                <option value="high" <?php selected(
                                    $curThoroughness,
                                    "high",
                                ); ?>><?php echo esc_html($this->tr("Many (40 messages)", "Viel (40 Nachrichten)")); ?></option>
                                <option value="custom" <?php selected(
                                    $curThoroughness,
                                    "custom",
                                ); ?>><?php echo esc_html($this->tr("Custom", "Benutzerdefiniert")); ?></option>
                            </select>
                        ...
                        <p class="levi-form-help"><?php echo esc_html(
                            $this->tr(
                    ...
                    <div class="levi-form-group">
                        <label class="levi-form-label" for="levi_safety_mode"><?php echo esc_html(
                            $this->tr(
                                "Allow destructive actions?",
                                "Destruktive Aktionen erlauben?",
                            ),
                        ); ?></label>
                        <select id="levi_safety_mode" name="<?php echo esc_attr(
                            $this->optionName,
                        ); ?>[levi_safety_mode]" class="levi-form-select">
                            <option value="strict" <?php selected(
                                $curSafety,
                                "strict",
                            ); ?>><?php echo esc_html($this->tr("No — Levi cannot delete or remove anything (safer)", "Nein — Levi darf nichts loeschen oder entfernen (sicherer)")); ?></option>
                            <option value="standard" <?php selected(
                                $curSafety,
                                "standard",
                            ); ?>><?php echo esc_html($this->tr("Yes — Levi may delete posts, users, etc.", "Ja — Levi darf Beitraege, Benutzer usw. loeschen")); ?></option>
                        </select>
                        <p class="levi-form-help"><?php echo esc_html(
                            $this->tr(
                    ...
                    <div class="levi-form-group">
                        <label class="levi-form-label" for="levi_speed_mode"><?php echo esc_html(
                            $this->tr(
                                "How many work steps per request?",
                                "Wie viele Arbeitsschritte pro Anfrage?",
                            ),
                        ); ?></label>
                        <div style="display: flex; gap: 0.75rem; align-items: center;">
                            <select id="levi_speed_mode" name="<?php echo esc_attr(
                                $this->optionName,
                            ); ?>[levi_speed_mode]" class="levi-form-select" style="flex: 1;">
                                <option value="fast" <?php selected(
                                    $curSpeed,
                                    "fast",
                                ); ?>><?php echo esc_html($this->tr("Few (15 steps)", "Wenig (15 Schritte)")); ?></option>
                                <option value="balanced" <?php selected(
                                    $curSpeed,
                                    "balanced",
                                ); ?>><?php echo esc_html($this->tr("Standard (25 steps, recommended)", "Standard (25 Schritte, empfohlen)")); ?></option>
                                <option value="careful" <?php selected(
                                    $curSpeed,
                                    "careful",
                                ); ?>><?php echo esc_html($this->tr("Many (30 steps)", "Viel (30 Schritte)")); ?></option>
                                <option value="custom" <?php selected(
                                    $curSpeed,
                                    "custom",
                                ); ?>><?php echo esc_html($this->tr("Custom", "Benutzerdefiniert")); ?></option>
                ...
                <div class="levi-form-card">
                    <h3><?php echo esc_html(
                        $this->tr("AI Response", "KI-Antwort"),
                    ); ?></h3>
                    <div class="levi-form-row">
                        <div class="levi-form-group">
                            <label class="levi-form-label"><?php echo esc_html(
                                $this->tr("Max Tokens", "Max. Tokens"),
                            ); ?></label>
                            <input type="number"
                                   name="<?php echo esc_attr(
                                       $this->optionName,
                            ...
                            <p class="levi-form-help">
                                <?php echo esc_html(
                                    $this->tr(
                                        "Maximum tokens per AI response. Capped automatically per model. The AI only uses what it needs.",
                                        "Maximale Tokens pro KI-Antwort. Wird pro Modell automatisch begrenzt. Die KI nutzt nur so viel wie noetig.",
                                    ),
                                ); ?>
                            </p>
                        </div>
                        <div class="levi-form-group">
                            ...
                            <p class="levi-form-help">
                                <?php echo esc_html(
                                    $this->tr(
                                        "How long to wait for the AI provider to respond.",
                                        "Wie lange auf Antwort des KI-Anbieters gewartet wird.",
                                    ),
                                ); ?>
                            </p>
                        </div>
                    ...
                    <div class="levi-form-group">
                        <label class="levi-form-label"><?php echo esc_html(
                            $this->tr(
                                "PHP Time Limit (seconds)",
                                "PHP-Zeitlimit (Sekunden)",
                            ),
                        ); ?></label>
                        <input type="number"
                               name="<?php echo esc_attr(
                               ...
                               value="<?php echo esc_attr(
                                   $settings["php_time_limit"],
                               ); ?>"
                               min="0" class="levi-form-input levi-input-small">
                        <p class="levi-form-help">
                            <?php echo esc_html(
                                $this->tr(
                                    "PHP set_time_limit() for chat requests. 0 = unlimited. Increase for complex multi-step tasks.",
                                    "PHP set_time_limit() fuer Chat-Anfragen. 0 = unbegrenzt. Bei komplexen mehrstufigen Aufgaben erhoehen.",
                                ),
                            ); ?>
                        </p>
                    </div>
                ...
                <div class="levi-form-card">
                    <h3><?php echo esc_html(
                        $this->tr("Conversation Context", "Kontext-Verlauf"),
                    ); ?></h3>
                    <p class="levi-form-description" style="margin-bottom: 12px;">
                        <?php echo esc_html(
                            $this->tr(
                                "Input context limit is derived automatically from the selected model. Older messages are trimmed and summarized when exceeded.",
                                "Das Input-Kontextlimit wird automatisch aus dem gewaehlten Modell abgeleitet. Aeltere Nachrichten werden bei Ueberschreitung gekuerzt und zusammengefasst.",
                            ),
                        ); ?>
                    </p>
                    <div class="levi-form-group">
                        ...
                        <p class="levi-form-help">
                            <?php echo esc_html(
                                $this->tr(
                                    "Cheap model for compacting older messages when context limit is reached. Falls back to primary model on failure. Leave empty for provider default.",
                                    "Guenstiges Modell fuer die Komprimierung aelterer Nachrichten bei Kontext-Ueberschreitung. Bei Fehler wird automatisch das Hauptmodell verwendet. Leer lassen fuer Provider-Standard.",
                                ),
                            ); ?>
                        </p>
                    </div>
            ...
            <div class="levi-form-card" style="margin-top: 1.5rem;">
                ...
                <div class="levi-form-actions-inline" style="margin-bottom: 0.75rem;">
                    <button type="button" id="levi-clear-audit-log" class="levi-btn levi-btn-secondary">
                        ...
                        <?php echo esc_html(
                            $this->tr("Delete Audit Log", "Audit-Log loeschen"),
                        ); ?>
                    </button>
                        ...
                        <table class="widefat striped" style="margin-top: 0.25rem;">
                            <thead>
                                <tr>
                                    ...
                                    <th><?php echo esc_html(
                                        $this->tr("Summary", "Zusammenfassung"),
                                    ); ?></th>
                                    <th><?php echo esc_html(
                                        $this->tr("Arguments", "Argumente"),
                                    ); ?></th>
                                </tr>
                            </thead>
                            <tbody>
                            <?php foreach ($auditRows as $row): ?>
                                <tr>
                                    ...
                                    <td><?php echo !empty($row["success"])
                                        ? "✅"
                                        : "❌"; ?></td>
                                    <td><?php echo esc_html(
                                        (string) ($row["result_summary"] ?? ""),
                                    ); ?></td>
                                    <td><code style="white-space:pre-wrap; word-break:break-word;"><?php echo esc_html(
                                        (string) ($row["tool_args"] ?? ""),
                                    ); ?></code></td>
                                </tr>
                            <?php endforeach; ?>
                            </tbody>
                    ...
                    <p class="levi-form-help" style="margin-top: 0.5rem;">
                        <?php echo esc_html(
                            $this->tr(
                                ...
                                "Es werden die letzten 20 Eintraege angezeigt.",
                            ),
                        ); ?>
                    </p>
                <?php endif; ?>
        ...
        <div class="levi-settings-section">
            ...
            <div class="levi-cards-grid">
                ...
                <div class="levi-form-card">
                    ...
                    <p class="levi-form-description">
                        <?php _e(
                            "Repair and maintain database tables.",
                            "levi-agent",
                        ); ?>
                    </p>
                    <p class="levi-form-help levi-hint">
                        <?php _e(
                            "Hint: Use this if conversations or settings behave incorrectly. It recreates Levi’s database tables without deleting existing data.",
                            "levi-agent",
                        ); ?>
                    </p>
                    ...
                    <div class="levi-form-group">
                        <label class="levi-toggle-label">
                            ...
                            <input type="checkbox"
                                   ...
                                   value="1"
                                   <?php checked(
                                       !empty($settings["web_search_enabled"]),
                                   ); ?>
                                   class="levi-toggle-input">
                            <span class="levi-toggle-switch"></span>
                            <span class="levi-toggle-text"><?php echo esc_html(
                                $this->tr(
                                    "Enable Web Search",
                ...
                <div class="levi-form-card">
                    ...
                    <div class="levi-card-content">
                        <div class="levi-status-row">
                            <span class="levi-status-label"><?php _e(
                                "Plugin Version",
                                "levi-agent",
                            ); ?></span>
                            <span class="levi-status-value"><?php echo esc_html(
    ...
    public function getProvider(): string
    {
        ...
        $provider = sanitize_key(
            (string) ($settings["ai_provider"] ?? "openrouter"),
        );
    ...
    public function getAuthMethodOptions(string $provider): array
    {
        if ($provider === "openrouter") {
            return [
                "oauth" => $this->tr(
                    "Connect with OpenRouter (recommended)",
                    "Mit OpenRouter verbinden (empfohlen)",
                ),
                "api_key" => $this->tr(
                    "Manual API Key",
                    "Manueller API-Schluessel",
                ),
            ];
        }
        return [
    ...
    public function getAuthMethod(): string
    {
        ...
        $provider = $this->getProvider();
        $method = sanitize_key(
            (string) ($settings["ai_auth_method"] ?? "api_key"),
        );
    ...
    public function getSettings(): array
    {
        ...
        if (
            isset($settings["require_confirmation_destructive"]) &&
            !isset($settings["allow_destructive"])
        ) {
            $settings["allow_destructive"] = empty(
                $settings["require_confirmation_destructive"]
            )
                ? 1
                : 0;
        }
    ...
    public function ajaxClearAuditLog(): void
    {
        ...
        if ($deleted === false) {
            wp_send_json_error(
                $this->tr(
                    "Could not delete audit log.",
                    "Audit-Log konnte nicht geloescht werden.",
                ),
            );
        }
    ...
    private function renderCronTasksTab(): void
    {
        ...
        <div class="levi-settings-section">
            <div class="levi-section-header">
                <h2><?php echo esc_html(
                    $this->tr("Levi's Tasks", "Levis Aufgaben"),
                ); ?></h2>
                <p><?php echo esc_html(
                    $this->tr(
                    ...
                    <table class="levi-data-table levi-cron-table">
                        <thead>
                            <tr>
                                ...
                                <th><?php echo esc_html(
                                    $this->tr("Interval", "Intervall"),
                                ); ?></th>
                                <th><?php echo esc_html(
                                    $this->tr("Last Run", "Letzter Lauf"),
                                ); ?></th>
                                <th><?php echo esc_html(
                                    $this->tr("Next Run", "Nächster Lauf"),
                                ); ?></th>
                                <th><?php echo esc_html(
                                    $this->tr("Status", "Status"),
                                ); ?></th>
                                <th><?php echo esc_html(
                                    $this->tr("Actions", "Aktionen"),
                                ); ?></th>
                            </tr>
                        </thead>
                        <tbody>
                        <?php foreach ($tasks as $task):
                            ...
                            $hasError =
                                $lastResult &&
                                str_contains($lastResult, '"success":false');
                            $isWriteTask = in_array(
                                $task["tool"] ?? "",
                                \Levi\Agent\Cron\CronTaskRunner::CONFIRMABLE_TOOLS,
                                true,
                            );
                            ...
                            $statusLabel = $isActive
                                ? ($hasError
                                    ? $this->tr("Error", "Fehler")
                                    : $this->tr("Active", "Aktiv"))
                                : $this->tr("Paused", "Pausiert");
                            ?>
                            <tr data-task-id="<?php echo esc_attr(
                                $task["id"],
                            ); ?>">
                                <td>
                                    <strong><?php echo esc_html(
                                ...
                                <td>
                                    ...
                                    <?php if ($isWriteTask): ?>
                                        <span class="levi-badge levi-badge-confirmed" title="<?php echo esc_attr(
                                            $this->tr(
                                                "Write tool — confirmed by user",
                                                "Schreib-Tool — vom Nutzer bestätigt",
                                            ),
                                        ); ?>">write</span>
                                    <?php endif; ?>
                                </td>
                                ...
                                <td>
                                    <span class="levi-badge <?php echo esc_attr(
                                        $statusClass,
                                    ); ?>"><?php echo esc_html(
    $statusLabel,
); ?></span>
                                ...
                                <td class="levi-cron-actions">
                                    <button type="button" class="levi-btn levi-btn-small levi-btn-secondary levi-cron-run" data-task-id="<?php echo esc_attr(
                                        $task["id"],
                                    ); ?>" title="<?php echo esc_attr(
    $this->tr("Run now", "Jetzt ausführen"),
); ?>">
                                        <span class="dashicons dashicons-controls-play"></span>
                                    </button>
                                    <button type="button" class="levi-btn levi-btn-small levi-btn-secondary levi-cron-toggle" data-task-id="<?php echo esc_attr(
                                        $task["id"],
                                    ); ?>" title="<?php echo esc_attr(
    $isActive
        ? $this->tr("Pause", "Pausieren")
        : $this->tr("Resume", "Fortsetzen"),
); ?>">
                                        <span class="dashicons <?php echo $isActive
                                            ? "dashicons-controls-pause"
                                            : "dashicons-controls-forward"; ?>"></span>
                                    </button>
                                    <button type="button" class="levi-btn levi-btn-small levi-btn-danger levi-cron-delete" data-task-id="<?php echo esc_attr(
                                        $task["id"],
                                    ); ?>" title="<?php echo esc_attr(
    $this->tr("Delete", "Löschen"),
); ?>">
                                        <span class="dashicons dashicons-trash"></span>
                                    </button>
                                    <?php $emailActive = !empty(
                                        $task["notify_email"]
                                    ); ?>
                                    <button type="button" class="levi-btn levi-btn-small <?php echo $emailActive
                                        ? "levi-btn-email-active"
                                        : "levi-btn-secondary"; ?> levi-cron-email-toggle" data-task-id="<?php echo esc_attr(
     $task["id"],
 ); ?>" title="<?php echo esc_attr(
    $emailActive
        ? $this->tr(
            "Email notifications on — click to disable",
            "E-Mail-Benachrichtigung aktiv — klicken zum Deaktivieren",
        )
        : $this->tr(
            "Enable email notifications",
            "E-Mail-Benachrichtigung aktivieren",
                                    ...
                                    <?php if ($lastResult): ?>
                                        <button type="button" class="levi-btn levi-btn-small levi-btn-secondary levi-cron-expand" data-task-id="<?php echo esc_attr(
                                            $task["id"],
                                        ); ?>" title="<?php echo esc_attr(
    $this->tr("Show result", "Ergebnis anzeigen"),
); ?>">
                            ...
                            <?php if ($lastResult): ?>
                            <tr class="levi-cron-detail" data-detail-for="<?php echo esc_attr(
                                $task["id"],
                            ); ?>" style="display:none;">
                                <td colspan="7">
        ...
        <div class="levi-settings-section levi-cron-all-events">
            <div class="levi-section-header levi-collapsible-header" id="levi-cron-all-toggle">
                <h2>
                    ...
                    <?php echo esc_html(
                        $this->tr(
                            "All WordPress Cron Events",
                            "Alle WordPress Cron-Events",
                        ),
                    ); ?>
                </h2>
                <p><?php echo esc_html(
                    ...
                    <table class="levi-data-table">
                        <thead>
                            <tr>
                                <th>Hook</th>
                                <th><?php echo esc_html(
                                    $this->tr("Schedule", "Zeitplan"),
                                ); ?></th>
                                <th><?php echo esc_html(
                                    $this->tr("Next Run", "Nächste Ausführung"),
                                ); ?></th>
                                <th><?php echo esc_html(
                                    $this->tr("Status", "Status"),
                                ); ?></th>
                        ...
                        <tbody>
                        <?php foreach (
                            array_slice($allEvents, 0, 100)
                            as $event
                        ): ?>
                            <tr class="<?php echo $event["is_levi"]
                                ? "levi-row-featured"
                                : ""; ?>">
                                <td><code><?php echo esc_html(
                                    $event["hook"],
                                ...
                                <td><?php echo esc_html(
                                    $scheduleLabels[$event["schedule"]] ??
                                        $event["schedule"],
                                ); ?></td>
                                <td><?php echo esc_html(
                                    wp_date("d.m.Y H:i:s", $event["timestamp"]),
                                ); ?></td>
                                <td>
                                    <?php if ($event["overdue"]): ?>
                                        <span class="levi-badge levi-badge-warning"><?php echo esc_html(
                                            $this->tr("overdue", "überfällig"),
                                        ); ?></span>
                                    <?php else: ?>
                                        <span class="levi-badge levi-status-active"><?php echo esc_html(
                                            $this->tr("scheduled", "geplant"),
                                        ); ?></span>
                                    <?php endif; ?>
                                </td>
                            </tr>
    ...
    public function ajaxRunCronTask(): void
    {
        ...
        if (!empty($result["success"])) {
            ...
            wp_send_json_error(
                $result["error"] ??
                    $this->tr("Task failed.", "Aufgabe fehlgeschlagen."),
            );
        }
    }
    ...
    public function renderDashboardWidget(): void
    {
        ...
        if (empty($tasks)) {
            echo '<p class="levi-dw-empty">Keine geplanten Aufgaben konfiguriert. <a href="' .
                esc_url(
                    admin_url(
                        "admin.php?page=levi-agent-settings&tab=cron-tasks",
                    ),
                ) .
                '">Aufgaben verwalten</a></p>';
            return;
        }

        $tasksWithResults = array_filter(
            $tasks,
            fn($t) => !empty($t["last_result"]),
        );
        usort(
            $tasksWithResults,
            fn($a, $b) => ($b["last_run"] ?? 0) <=> ($a["last_run"] ?? 0),
        );
        ...
        foreach ($tasksWithResults as $task) {
            ...
            $lastRun = !empty($task["last_run"])
                ? wp_date("d.m.Y H:i", $task["last_run"])
                : "—";
            ...
            echo '<div class="levi-dw-detail" id="levi-dw-detail-' .
                $taskId .
                '" style="display:none;">';
            echo '<pre class="levi-dw-raw">' .
                esc_html($this->formatJsonForDisplay($rawResult)) .
                "</pre>";
            echo "</div>";
            echo "</div>";
        }
        ...
        echo '<p class="levi-dw-footer"><a href="' .
            esc_url(
                admin_url("admin.php?page=levi-agent-settings&tab=cron-tasks"),
            ) .
            '">Alle Aufgaben verwalten →</a></p>';
    ...
    private function buildResultSummary(?array $decoded, string $raw): string
    {
        if (!is_array($decoded)) {
            ...
            return '<span class="levi-dw-text">' .
                esc_html($excerpt) .
                (mb_strlen($raw) > 120 ? "…" : "") .
                "</span>";
        }
        ...
        if (!empty($decoded["error"])) {
            return '<span class="levi-dw-text levi-dw-error-text">Fehler: ' .
                esc_html(mb_substr($decoded["error"], 0, 150)) .
                "</span>";
        }
    ...
    public function ajaxToggleCronEmail(): void
    {
        ...
        if (!isset($tasks[$taskId])) {
            wp_send_json_error(
                $this->tr("Task not found.", "Aufgabe nicht gefunden."),
            );
        }

        $tasks[$taskId]["notify_email"] = empty(
            $tasks[$taskId]["notify_email"]
        );
