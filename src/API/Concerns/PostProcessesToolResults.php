...
trait PostProcessesToolResults
{
     ...
     */
    public function injectPostWriteValidation(
        array $toolCalls,
        array $toolResults,
    ): array {
        $hadWrite = false;
     ...
     */
    public function injectPostPatchVerification(
        array $toolCalls,
        array $toolResults,
    ): array {
        ...
        foreach ($toolResults as $tr) {
            ...
            $slug =
                $tr["result"]["plugin_slug"] ??
                ($tr["result"]["theme_slug"] ?? "?");
        ...
        return [
            [
                "role" => "system",
                "content" =>
                    "[PATCH-VERIFIKATION] Erfolgreich gepatcht: {$fileList}.\n\n" .
     ...
     */
    public function injectPostCSSWriteNudge(
        array $toolCalls,
        array $toolResults,
    ): array {
        $cssWritten = false;
        foreach ($toolResults as $tr) {
            ...
            $path =
                $tr["result"]["path"] ?? ($tr["result"]["relative_path"] ?? "");
            if ($path === "") {
                $args = [];
                foreach ($toolCalls as $tc) {
        ...
        if ($httpFetchTool && $httpFetchTool->checkPermission()) {
            ...
            return [
                ...
                [
                    ...
                    "tool_call_id" => $fakeToolCallId,
                    "content" =>
                        $this->compactToolResultForModel($fetchResult) .
        ...
        return [
            [
                "role" => "system",
                "content" =>
                    "[SYSTEM] Du hast eine CSS/JS-Datei geschrieben, aber http_fetch ist nicht verfuegbar. " .
     ...
     */
    public function injectPostPluginSmokeTest(
        array $toolCalls,
        array $toolResults,
    ): array {
        $pluginSlug = null;
            ...
            if (is_wp_error($activation)) {
                ...
                return [
                    ...
                    [
                        ...
                        "tool_call_id" => $fakeId,
                        "content" =>
                            "[SMOKE-TEST] Plugin '$pluginSlug' konnte nicht aktiviert werden: " .
        ...
        if (is_dir($pluginDir)) {
            try {
                ...
                foreach ($iterator as $file) {
                    ...
                    if (
                        preg_match_all(
                            "/register_post_type\s*\(\s*['\"]([a-z0-9_]+)['\"]/i",
                            $content,
                            $m,
                        )
                    ) {
        ...
        if (function_exists("wc_get_page_id")) {
            ...
            $writtenPhpParts = $this->collectWrittenPhpContent(
                $toolCalls,
                $toolResults,
            );
            $writtenCode = implode("\n", $writtenPhpParts);
            if ($writtenCode !== "") {
                $hasCartRef = (bool) preg_match(
                    "/woocommerce_(before_cart|after_cart|cart_contents|after_cart_table|before_cart_table|cart_coupon|cart_actions|cart_totals)/",
                    $writtenCode,
                );
                $hasCheckoutRef = (bool) preg_match(
                    "/woocommerce_(before_checkout|after_checkout|checkout_before|checkout_after|review_order)/",
                    $writtenCode,
                );
        ...
        if (is_wp_error($fetchResponse)) {
            $hasFatalError = true;
            $errorDetail =
                "HTTP-Fehler: " . $fetchResponse->get_error_message();
        } else {
            ...
            if (
                preg_match(
                    "/Fatal\s+error:.*?in\s+.*?on\s+line\s+\d+/is",
                    $body,
                    $m,
                )
            ) {
                ...
                preg_match(
                    "/(?:Uncaught\s+(?:Error|Exception|TypeError|ArgumentCountError)[^<]*)/is",
                    $body,
                    $m,
                )
            ) {
                ...
                stripos($body, "critical error on your website") !== false ||
                stripos($body, "kritischer Fehler") !== false
            ) {
                $hasFatalError = true;
        ...
        $errorLogResult = $this->toolRegistry->execute("read_error_log", [
            "lines" => 30,
            "filter" =>
                "Fatal|Warning|Deprecated|ArgumentCountError|TypeError|Uncaught",
        ]);
        $recentLogErrors = "";
        ...
        if ($hasFatalError) {
            ...
            $errorContent =
                "[SMOKE-TEST FEHLGESCHLAGEN] Plugin '$pluginSlug' verursacht einen Runtime-Fehler!\n\n" .
        ...
        $okContent =
            "[SMOKE-TEST BESTANDEN] Plugin '$pluginSlug' wurde aktiviert und die Seite $testUrl " .
        ...
        foreach ($additionalUrls as $extraUrl) {
            ...
            if (!is_wp_error($extraResponse)) {
                ...
                if (
                    preg_match(
                        "/Fatal\s+error:|Uncaught\s+(?:Error|Exception)|critical error/i",
                        $extraBody,
                    )
                ) {
                    $extraHasError = true;
     ...
     */
    public function injectPostCreatePluginNudge(
        array $toolCalls,
        array $toolResults,
    ): array {
        $createdSlug = null;
        ...
        foreach ($toolCalls as $tc) {
            ...
            foreach ($toolResults as $tr) {
                if (
                    $tr["tool"] === "create_plugin" &&
                    ($tr["result"]["success"] ?? false)
                ) {
                    $createdSlug = $tr["result"]["slug"] ?? null;
                    $pluginType =
                        $tr["result"]["plugin_type"] ??
                        ($args["plugin_type"] ?? "plain");
                    break 2;
                }
            }
        }
        ...
        $nudge =
            "[SYSTEM – PFLICHT] Das Plugin '$createdSlug' wurde als LEERES SCAFFOLD erstellt. " .
     ...
     */
    private function buildEnvironmentHintsForPluginCreation(
        string $pluginType,
    ): string {
        $env = \Levi\Agent\Memory\StateSnapshotService::getCachedEnvironment();
        $lastData = get_option("levi_agent_state_snapshot_last", []);
        $snapshot = is_array($lastData["snapshot"] ?? null)
            ? $lastData["snapshot"]
            : [];
        $hints = [];
        ...
        if ($isBlockTheme) {
            $hints[] =
                "[THEME] Das aktive Theme '$themeName' ist ein Block-Theme (FSE). " .
        ...
        if (!empty($env["elementor_active"])) {
            $hints[] =
                "[ELEMENTOR] Elementor ist aktiv. Seiten koennen mit Elementor gebaut sein. " .
                "Pruefe vor the_content-Filtern ob die Zielseite Elementor nutzt.";
        }

        $phpVersion = (string) ($snapshot["php_version"] ?? PHP_VERSION);
        $phpMajorMinor = implode(
            ".",
            array_slice(explode(".", $phpVersion), 0, 2),
        );
    ...
    private function findMainPluginFile(string $pluginSlug): ?string
    {
        ...
        foreach ($entries as $entry) {
            ...
            if (
                $header !== false &&
                preg_match("/Plugin\s+Name\s*:/i", $header)
            ) {
                return $path;
            }
        }
     ...
     */
    private function collectWrittenPhpContent(
        array $toolCalls,
        array $toolResults,
    ): array {
        $writtenPhpContent = [];
     ...
     */
    public function injectPostWriteEnvironmentWarnings(
        array $toolCalls,
        array $toolResults,
    ): array {
        $writtenPhpContent = $this->collectWrittenPhpContent(
            $toolCalls,
            $toolResults,
        );
        ...
        if ($cartIsBlock || $checkoutIsBlock) {
            $cartHookPattern =
                "/woocommerce_(before_cart|after_cart|cart_contents|before_cart_table|after_cart_table|cart_coupon|cart_actions|before_cart_totals|cart_totals_before_order_total)/";
            $checkoutHookPattern =
                "/woocommerce_(before_checkout_form|after_checkout_form|checkout_before_customer_details|checkout_after_customer_details|review_order_before_payment)/";
            ...
            if (
                $cartIsBlock &&
                preg_match_all($cartHookPattern, $allCode, $m)
            ) {
                $foundCartHooks = array_unique($m[0]);
            }
            if (
                $checkoutIsBlock &&
                preg_match_all($checkoutHookPattern, $allCode, $m)
            ) {
            ...
            if (!empty($foundCartHooks)) {
                $warnings[] =
                    "[ENVIRONMENT-KONFLIKT] Dein Code verwendet klassische Cart-Hooks (" .
                    implode(", ", $foundCartHooks) .
                    "), aber der Warenkorb nutzt WooCommerce BLOCKS. " .
            ...
            if (!empty($foundCheckoutHooks)) {
                $warnings[] =
                    "[ENVIRONMENT-KONFLIKT] Dein Code verwendet klassische Checkout-Hooks (" .
                    implode(", ", $foundCheckoutHooks) .
                    "), aber der Checkout nutzt WooCommerce BLOCKS. " .
                    "Diese Hooks feuern NICHT auf Block-basierten Seiten! " .
        ...
        if ($isBlockTheme) {
            ...
            if (!empty($foundClassicPatterns)) {
                $warnings[] =
                    "[ENVIRONMENT-HINWEIS] Dein Code verwendet Classic-Theme-Funktionen (" .
                    implode(", ", $foundClassicPatterns) .
                    "), aber das aktive Theme ist ein Block-Theme (FSE). " .
        ...
        if (!empty($env["elementor_active"])) {
            if (
                preg_match("/the_content\s*\(/", $allCode) &&
                str_contains($allCode, "add_filter")
            ) {
                $warnings[] =
                    "[ENVIRONMENT-HINWEIS] Elementor ist aktiv. the_content Filter koennen mit " .
        ...
        if (
            preg_match('/add_filter\s*\(\s*[\'"]the_content[\'"]/', $allCode) &&
            preg_match("/\bthe_content\s*\(/", $allCode)
        ) {
            $warnings[] =
                "[CODE-FEHLER] Dein Code registriert einen the_content-Filter UND ruft the_content() " .
     ...
     */
    private function checkMetaKeyConsistency(
        string $writtenCode,
        array $toolResults,
        array &$warnings,
    ): void {
        ...
        foreach ($onlyInGet as $getKey) {
            foreach ($onlyInUpdate as $updateKey) {
                $shorter =
                    strlen($getKey) < strlen($updateKey) ? $getKey : $updateKey;
                $longer =
                    strlen($getKey) >= strlen($updateKey)
                        ? $getKey
                        : $updateKey;
                if (
                    str_contains($longer, $shorter) ||
                    similar_text($getKey, $updateKey) > strlen($longer) * 0.6
                ) {
                    $warnings[] =
                        "[META-KEY MISMATCH] get_post_meta liest '{$getKey}', aber update_post_meta schreibt '{$updateKey}'. " .
    ...
    public function injectPostWriteIntegrationCheck(
        array $toolCalls,
        array $toolResults,
    ): array {
        $writtenSubFiles = [];
        ...
        foreach ($writtenSubFiles as $sub) {
            ...
            if ($ext === "php") {
                ...
                $escaped = preg_quote($basename, "/");
                $enqueuePattern =
                    "/wp_(?:enqueue|register)_(?:style|script)\s*\(.*?" .
                    $escaped .
                    "/is";
                ...
                if (!$found) {
                    ...
                    if ($allPhpFiles) {
                        foreach ($allPhpFiles as $phpFile) {
                            ...
                            if (
                                $otherContent !== false &&
                                preg_match($enqueuePattern, $otherContent)
                            ) {
                                $found = true;
                                break;
                            }
                ...
                $typeHint =
                    $ext === "php"
                        ...
                        : ($ext === "css"
                            ...
                            : "wp_enqueue_script");
                $warnings[] =
                    "'{$relPath}' in Plugin '{$slug}' wird von keiner PHP-Datei eingebunden " .
     ...
     */
    public function injectAutoListOnMissingFile(
        array $toolCalls,
        array $toolResults,
    ): array {
        $injected = [];
        ...
        foreach ($toolResults as $i => $tr) {
            ...
            foreach ($toolCalls as $tc) {
                if (($tc["function"]["name"] ?? "") === $tool) {
                    $args =
                        ...
                        [];
                    $slug =
                        $args["plugin_slug"] ?? ($args["theme_slug"] ?? null);
                    break;
                }
            }
            if ($slug === null) {
            ...
            $listTool = $isPluginRead
                ? "list_plugin_files"
                : "list_theme_files";
            $slugParam = $isPluginRead ? "plugin_slug" : "theme_slug";

            $listResult = $this->toolRegistry->execute($listTool, [
                $slugParam => $slug,
                "max_depth" => 3,
            ]);
            ...
            $injected[] = [
                ...
                "tool_call_id" => $fakeId,
                "content" =>
                    "[AUTO-RECOVERY] Die Datei existiert nicht. Hier ist die tatsaechliche Dateistruktur von '$slug':\n\n" .
                    $fileTree .
     ...
     */
    public function injectCodeTagWarnings(array $toolResults): array
    {
        $affectedFiles = [];
        ...
        foreach ($toolResults as $tr) {
            ...
            $slug =
                ...
                ($tr["result"]["theme_slug"] ?? "?");
            $relPath = $tr["result"]["relative_path"] ?? "?";
            $patchTool = str_contains($tool, "theme")
                ? "write_theme_file"
                : "patch_plugin_file";
     ...
     */
    public function injectToolMismatchCorrection(
        ?string $userMessage,
        array $toolResults,
    ): array {
        if (
        ...
        $calledTools = array_unique(
            array_map(fn($tr) => $tr["tool"] ?? "", $toolResults),
        );

        if (
            $userMeansPost &&
            in_array("get_pages", $calledTools, true) &&
            !in_array("get_posts", $calledTools, true)
        ) {
            return [
                [
                    "role" => "system",
                    "content" =>
                        "[SYSTEM – FALSCHES TOOL VERWENDET]\n" .
     ...
     */
    public function checkWriteCompleteness(array $toolResults): ?string
    {
        $pluginWrites = [];
        ...
        foreach ($pluginWrites as $slug => $writtenPaths) {
            ...
            foreach ($writtenPaths as $relPath) {
                ...
                if ($ext === "php") {
                    $escaped = preg_quote($basename, "/");
                    $allPhpFiles =
                        glob($pluginRoot . "/{,*/,*/*/}*.php", GLOB_BRACE) ?:
                        [];
                    foreach ($allPhpFiles as $phpFile) {
                ...
                if (!$found) {
                    $issues[] =
                        "Plugin '{$slug}': Datei '{$relPath}' wurde geschrieben, " .
            ...
            if (
                preg_match_all(
                    '/(?:require|include)(?:_once)?\s*[\(]?\s*(?:__DIR__|dirname\s*\(\s*__FILE__\s*\))\s*\.\s*[\'"]([^"\']+)[\'"]/i',
                    $mainContent,
                    $matches,
                )
            ) {
                foreach ($matches[1] as $referenced) {
                    ...
                    if (!is_file($fullPath)) {
                        $issues[] =
                            "Plugin '{$slug}': Die Hauptdatei referenziert '{$referenced}', " .
     ...
     */
    public function injectPostWriteReverseDependencyWarnings(
        array $toolCalls,
        array $toolResults,
    ): array {
        $writeTools = [
        ...
        foreach ($toolResults as $tr) {
            ...
            $symbols = $this->extractChangedSymbols(
                $tool,
                $toolCalls,
                $rootDir,
                $relPath,
        ...
        foreach ($changedFiles as $cf) {
            $depHits = $this->scanReverseDependencies(
                $cf["root"],
                $cf["rel_path"],
                $cf["symbols"],
            );
            ...
            foreach ($depHits as $symbol => $refs) {
                ...
                $lines[] =
                    "- `{$symbol}` wird verwendet in: " .
                    implode(", ", $refParts);
                if (count($refs) > 5) {
                    $lines[count($lines) - 1] .=
                        " (+" . (count($refs) - 5) . " weitere)";
                }
            }

            $allWarnings[] =
                "[DEPENDENCY-WARNUNG] {$cf["rel_path"]} enthaelt Symbole, die in anderen Dateien referenziert werden:\n" .
                implode("\n", $lines) .
     ...
     */
    public function injectPostWriteReferenceCheck(
        array $toolCalls,
        array $toolResults,
    ): array {
        $writeTools = [
        ...
        foreach ($toolResults as $tr) {
            ...
            if (
                method_exists($this, "checkReferenceIntegrity") &&
                $content !== false
            ) {
                if (
                    !isset($knownFunctionsCache[$rootDir]) &&
                    method_exists($this, "buildKnownFunctionsForPlugin")
                ) {
                    $knownFunctionsCache[
                        $rootDir
                    ] = $this->buildKnownFunctionsForPlugin($rootDir);
                }
                $refCheck = $this->checkReferenceIntegrity(
                    $filePath,
                    $rootDir,
                    $knownFunctionsCache[$rootDir] ?? null,
                );
                if (!empty($refCheck["undefined_calls"])) {
                    $preview = implode(
                        ", ",
                        array_slice($refCheck["undefined_calls"], 0, 5),
                    );
            ...
            if (
                method_exists($this, "checkWordPressPatterns") &&
                $content !== false
            ) {
            ...
            if (!empty($fileWarnings)) {
                $allWarnings[] =
                    "[CODE-WARNUNG] Moegliche Probleme in {$relPath}:\n- " .
                    implode("\n- ", $fileWarnings) .
    ...
    private function extractChangedSymbols(
        string $tool,
        array $toolCalls,
        string $rootDir,
        string $relPath,
        ...
        if (
            ...
            foreach ($m[1] as $hook) {
                if (
                    ...
                    !str_starts_with($hook, "admin_") &&
                    !str_starts_with($hook, "init")
                ) {
                    $symbols[] = $hook;
                }
            }
     ...
     */
    private function scanReverseDependencies(
        string $rootDir,
        string $changedRelPath,
        array $symbols,
    ): array {
        if (empty($symbols)) {
            return [];
        ...
        try {
            ...
            foreach ($iterator as $file) {
                ...
                if (
                    in_array($ext, $skipExtensions, true) ||
                    !in_array($ext, ["php", "js", "css"], true)
                ) {
                    continue;
                }
    ...
    private function detectWcBlocksInBody(string $body): string
    {
        ...
        if (
            str_contains($body, "wp-block-woocommerce-cart") ||
            str_contains($body, "wc-block-cart")
        ) {
            $blockPages[] = "Cart";
        }
        if (
            str_contains($body, "wp-block-woocommerce-checkout") ||
        ...
        return "[WC-ENVIRONMENT] Diese Seite nutzt WooCommerce Blocks (" .
            implode(", ", $blockPages) .
            "). " .
