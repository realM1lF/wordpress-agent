...
- **Patches sind jetzt atomar:** Wenn Levi mehrere Änderungen in einer Datei vornimmt und auch nur eine davon fehlschlägt (z. B. weil der Suchtext nicht exakt matcht), wird gar nichts geschrieben. Kein Teilresultat mehr — entweder alles klappt oder nichts. Levi bekommt eine klare Fehlermeldung mit Vorschlägen zur Korrektur.
- **Verification Context nach jedem Patch:** Nach erfolgreichen Patches zeigt Levi jetzt die Zeilen rund um jede Änderung. Ein Post-Processor zwingt ihn, diesen Kontext zu prüfen — so merkt er schneller, wenn etwas fehlt oder nicht wie beabsichtigt aussieht.
- **Multi-Datei-Lesen wird nicht mehr abgeschnitten:** Wenn Levi mehrere Plugin-Dateien auf einmal liest (z. B. 3 CSS/JS-Dateien), wird jede Datei einzeln mit einem fairen Budget gekürzt — statt dass die Mitte aller Dateien blind wegfällt. Levi muss nicht mehr mehrfach lesen, weil der Inhalt abgeschnitten war.
- **Tool-Namen lesbar in der Timeline:** Statt roher Tool-Namen wie `read_plugin_file` siehst du jetzt „Plugin-Datei lesen…“ und ähnliche verständliche Labels — auch schon in der Vorschau, bevor der Aufruf fertig ist.
